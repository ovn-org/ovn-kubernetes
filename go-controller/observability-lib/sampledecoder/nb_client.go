...
const OVSDBTimeout = 100 * time.Second
...
func NewOVSDBClientWithConfig(ctx context.Context, cfg nbConfig) (client.Client, error) {
	dbModel, err := ovsdb.FullDatabaseModel()
	if err != nil {
		return nil, err
	}
...
func newClient(cfg nbConfig, dbModel model.ClientDBModel) (client.Client, error) {
