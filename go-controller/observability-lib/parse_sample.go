...
func (r *SampleReader) ReadSamples(ctx context.Context) error {
	if r.enableDecoder {
		...
		if r.addOVSCollector {
			r.decoder, err = sampledecoder.NewSampleDecoderWithDefaultCollector(ctx, "ovnk-debug", 123)
			if err != nil {
				return fmt.Errorf("error creating decoder: %w", err)
			}
			defer r.decoder.Shutdown()
		} else {
			r.decoder, err = sampledecoder.NewSampleDecoder(ctx)
			if err != nil {
				return fmt.Errorf("error creating decoder: %w", err)
			}
		}
