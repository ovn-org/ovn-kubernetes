...
func (r *RowCache) rowsByCondition(conditions []ovsdb.Condition, cloner cloner) (map[string]model.Model, error) {
	...
	if len(conditions) == 0 {
		for uuid := range r.cache {
			results[uuid] = cloner(r.rowByUUID(uuid))
		}
		return results, nil
	}
...
type eventProcessor struct {
	events chan event
	// eventMutex locks the handlers array when we add a handler or dispatch events
...
func (e *eventProcessor) Run(stopCh <-chan struct{}) {
	e.eventMutex.Lock()
	// lazy init the event channel as it has performance cost and not everyone
	// has a use for it
	if e.events == nil {
