...
func main() {
	...
	dumpUserDefinedNetworksVRFTableIDsForNetwork := flag.String("dump-udn-vrf-table-ids", "", "Dump the VRF table ID per node for specified user defined network")
	...
	if dumpUserDefinedNetworksVRFTableIDsForNetwork != nil {
		nodesVRFTableIDs, err := findUserDefinedNetworkVRFTableIDs(coreclient, restconfig, ovnNamespace, *dumpUserDefinedNetworksVRFTableIDsForNetwork)
		if err != nil {
			klog.Exitf("Failed dumping VRF table IDs: %s", err)
		}
		tableIDs, err := json.Marshal(nodesVRFTableIDs)
		if err != nil {
			klog.Exitf("Failed dumping VRF table IDs: %s", err)
		}
		fmt.Println(string(tableIDs))
		return
	}
