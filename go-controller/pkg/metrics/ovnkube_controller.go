	...
	go func() {
		for i := 0; i < maxAttemptForIPsecQuery; i++ {
			...
			default:
				...
				stdout, stderr, err := util.RunIP("xfrm", "state", "show", "|", "grep", "-i", "offload", "-wc")
				if err != nil {
				...
				if err != nil {
					klog.Errorf("Error converting IPsec offload state %s: %v", stdout, err)
					return
				}
				if count == 0 {
					// can we return early enough if primary interface doesn't have support for esp offload ?
					metricIPsecOffloadEnabled.Set(0)
					time.Sleep(ipSecQueryWaitTime)
					continue
				}
