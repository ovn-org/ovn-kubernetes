...
func NewOVNKubeControllerWatchFactory(ovnClientset *util.OVNKubeControllerClientset) (*WatchFactory, error) {
	...
	if config.OVNKubernetesFeature.EnableMultiNetwork {
		wf.informers[IPAMClaimsType], err = newQueuedInformer(IPAMClaimsType, wf.ipamClaimsFactory.K8s().V1alpha1().IPAMClaims().Informer(), wf.stopChan, defaultNumEventQueues)
		if err != nil {
			return nil, err
		}
	}
