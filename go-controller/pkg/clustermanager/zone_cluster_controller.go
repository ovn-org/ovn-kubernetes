...
func (zcc *zoneClusterController) updateNodeAnnotationWithRetry(nodeName string,
	...
	resultErr := retry.RetryOnConflict(retry.DefaultBackoff, func() error {
		...
		if err != nil {
			return fmt.Errorf("unable to fetch nodes from watcher, err: %w", err)
		}

		cnode := node.DeepCopy()
		if cnode.Annotations == nil {
			cnode.Annotations = make(map[string]string)
		}
