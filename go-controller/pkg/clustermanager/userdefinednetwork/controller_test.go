		...
		Context("reconcile UDN CR", func() {
			...
			It("should fail when required namespace label is missing for primary network", func() {
				...
				Eventually(func() []metav1.Condition {
					...
					Status:  "False",
					Reason:  "SyncError",
					Message: "invalid primary network state for namespace \"test\": a valid primary UDN CR and required namespace label \"k8s.ovn.org/primary-user-defined-network\" must both be present",
				}}))
			...
			When("CR exist, and few connected & disconnected namespaces", func() {
				...
				It("when new namespace is created without required UDN label, it should not create NAD", func() {
					...
					Eventually(func() []metav1.Condition {
						...
						Message: "invalid primary network state for namespace \"black\": a valid primary UDN CR and required namespace label \"k8s.ovn.org/primary-user-defined-network\" must both be " +
							"present\ninvalid primary network state for namespace \"gray\": a valid primary UDN CR and required namespace label \"k8s.ovn.org/primary-user-defined-network\" must both be present",
