	...
	ra := &ratypes.RouteAdvertisements{
		...
		Spec: ratypes.RouteAdvertisementsSpec{
			...
			Advertisements: ratypes.Advertisements{
				PodNetwork: tra.AdvertisePods,
				EgressIP:   tra.AdvertiseEgressIPs,
			},
		},
	}
...
type testNeighbor struct {
	ASN               uint32
	Address           string
	ReceiveMode       frrapi.AllowMode
	AdvertiseMode     frrapi.AllowMode
	...
	n := frrapi.Neighbor{
		...
		ToAdvertise: frrapi.Advertise{
			Allowed: frrapi.AllowedOutPrefixes{
				Mode:     tn.AdvertiseMode,
				Prefixes: tn.AdvertisePrefixes,
			},
		},
	}
	if tn.ReceiveMode == "" {
		n.ToReceive.Allowed.Mode = frrapi.AllowRestricted
	}
	if tn.AdvertiseMode == "" {
		n.ToAdvertise.Allowed.Mode = frrapi.AllowRestricted
	}
	return n
}
...
func (tn testNAD) NAD() *nadtypes.NetworkAttachmentDefinition {
	...
	switch {
	case tn.IsSecondary:
		nad.Spec.Config = fmt.Sprintf("{\"cniVersion\": \"0.4.0\", \"name\": \"%s\", \"type\": \"%s\", \"topology\": \"%s\", \"netAttachDefName\": \"%s\", \"subnets\": \"192.168.0.0/16\"}",
			tn.Network,
			config.CNI.Plugin,
			topology,
			tn.Namespace+"/"+tn.Name)
	case tn.Topology != "":
		nad.Spec.Config = fmt.Sprintf("{\"cniVersion\": \"0.4.0\", \"name\": \"%s\", \"type\": \"%s\", \"topology\": \"%s\", \"netAttachDefName\": \"%s\", \"role\": \"primary\", \"subnets\": \"192.168.0.0/16\"}",
			tn.Network,
			config.CNI.Plugin,
			topology,
			tn.Namespace+"/"+tn.Name)
...
func TestController_reconcile(t *testing.T) {
	...
	tests := []struct {
			...
			frrConfigs: []*testFRRConfig{
				{
					...
					Routers: []*testRouter{
						{ASN: 1, Prefixes: []string{"1.1.1.0/24"}, Neighbors: []*testNeighbor{
							{ASN: 1, Address: "1.0.0.100", ReceiveMode: frrapi.AllowRestricted, AdvertiseMode: frrapi.AllowAll},
						}},
					},
				},
			},
			...
			frrConfigs: []*testFRRConfig{
				{
					...
					Routers: []*testRouter{
						{ASN: 1, Prefixes: []string{"1.1.1.0/24"}, Neighbors: []*testNeighbor{
							{ASN: 1, Address: "1.0.0.100", ReceiveMode: frrapi.AllowAll, AdvertiseMode: frrapi.AllowRestricted, AdvertisePrefixes: []string{"1.1.1.0/24"}},
						}},
					},
				},
			},
			...
			expectFRRConfigs: []*testFRRConfig{
				{
					...
					Routers: []*testRouter{
						{ASN: 1, Prefixes: []string{"1.2.0.0/24", "1.3.0.0/24"}, Imports: []string{"blue", "red"}, Neighbors: []*testNeighbor{
							{ASN: 1, Address: "1.0.0.100", ReceiveMode: frrapi.AllowRestricted, AdvertiseMode: frrapi.AllowRestricted, AdvertisePrefixes: []string{"1.2.0.0/24", "1.3.0.0/24"}},
						}},
			...
			frrConfigs: []*testFRRConfig{
				{
					...
					Routers: []*testRouter{
						{ASN: 1, VRF: "red", Prefixes: []string{"1.1.1.0/24"}, Neighbors: []*testNeighbor{
							{ASN: 1, Address: "1.0.0.100", ReceiveMode: frrapi.AllowAll, AdvertiseMode: frrapi.AllowAll},
						}},
					},
				},
			},
				...
				{ // not selected
					...
					Routers: []*testRouter{
						{ASN: 3, VRF: "blue", Prefixes: []string{"3.0.1.0/24"}, Neighbors: []*testNeighbor{
							{ASN: 3, Address: "3.0.0.100", ReceiveMode: frrapi.AllowAll, AdvertiseMode: frrapi.AllowAll},
						}},
					},
				},
			},
			nodes: []*testNode{
				{Name: "node1", Labels: map[string]string{"selected": "true", "node": "node1"}, SubnetsAnnotation: "{\"default\":\"1.1.0.0/24\", \"red\":\"1.2.0.0/24\", \"blue\":\"1.3.0.0/24\"}"},
				{Name: "node2", Labels: map[string]string{"selected": "true", "node": "node2"}, SubnetsAnnotation: "{\"default\":\"2.1.0.0/24\", \"red\":\"2.2.0.0/24\", \"blue\":\"2.3.0.0/24\"}"},
				{Name: "node3", SubnetsAnnotation: "{\"default\":\"3.1.0.0/24\", \"red\":\"3.2.0.0/24\", \"blue\":\"3.3.0.0/24\"}"}, // not selected
			},
			reconcile:            "ra",
			expectAcceptedStatus: metav1.ConditionTrue,
			expectFRRConfigs: []*testFRRConfig{
				{
					...
					Routers: []*testRouter{
						...
						{ASN: 1, VRF: "red", Prefixes: []string{"1.2.0.0/24"}, Neighbors: []*testNeighbor{
							{ASN: 1, Address: "1.0.0.100", ReceiveMode: frrapi.AllowRestricted, AdvertiseMode: frrapi.AllowAll},
						}},
					},
				},
				{
					...
					Routers: []*testRouter{
						{ASN: 1, Prefixes: []string{"2.1.0.0/24"}, Neighbors: []*testNeighbor{
							{ASN: 1, Address: "1.0.0.100", ReceiveMode: frrapi.AllowRestricted, AdvertiseMode: frrapi.AllowAll},
						}},
					},
				},
				{
					...
					NodeSelector: map[string]string{"kubernetes.io/hostname": "node2"},
					Routers: []*testRouter{
						{ASN: 1, VRF: "red", Prefixes: []string{"2.2.0.0/24"}, Neighbors: []*testNeighbor{
							{ASN: 1, Address: "1.0.0.100", ReceiveMode: frrapi.AllowRestricted, AdvertiseMode: frrapi.AllowAll},
						}},
					},
				},
		...
		},
		{
			...
			ra:   &testRA{Name: "ra", AdvertisePods: true, NetworkSelector: map[string]string{"selected": "true"}},
			nads: []*testNAD{
				{Name: "red", Namespace: "red", Network: "red", Topology: "layer2", Labels: map[string]string{"selected": "true"}},
			},
			reconcile:            "ra",
			expectAcceptedStatus: metav1.ConditionFalse,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			c := NewController(wf, fakeClientset)
			...
			if tt.expectNADAnnotations == nil {
				tt.expectNADAnnotations = map[string]map[string]string{}
			}
			g.Expect(actualNADAnnotations).To(gomega.Equal(tt.expectNADAnnotations))
		})
	}
}
...
func TestUpdates(t *testing.T) {
	testRAs := []*testRA{
		{
			...
			NodeSelector:             map[string]string{"select": "1"},
	...
	tests := []struct {
		...
		},
		{
			...
			newObject: &testNAD{Name: "net", Namespace: "net", Network: "net", Topology: "layer2", Labels: map[string]string{"select": "2"}},
		},
		{
			name:      "does not reconcile an updated unsupported NAD",
			oldObject: &testNAD{Name: "net", Namespace: "net", Network: "net", Topology: "layer2", Labels: map[string]string{"select": "2"}},
			newObject: &testNAD{Name: "net", Namespace: "net", Network: "net", Topology: "layer2", Labels: map[string]string{"select": "1"}},
		},
		{
