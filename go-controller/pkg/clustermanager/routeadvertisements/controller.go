...
func (c *Controller) ReconcileNetwork(name string, old, new util.NetInfo) {
	...
	// only need to signal the reconciliation on new networks and it is enough
	// to do so with a single of its NADs
	if new == nil || old != nil {
		return
	}
...
func (c *Controller) reconcileRouteAdvertisements(name string, ra *ratypes.RouteAdvertisements) (bool, error) {
	...
	if err != nil {
		return false, fmt.Errorf("failed to generate FRRConfigurations for RouteAdvertisements %q: %w", name, err)
	}
	...
	return hadFRRConfigUpdates || hadNADUpdates, nil
}
...
func (c *Controller) generateFRRConfigurations(ra *ratypes.RouteAdvertisements) ([]*frrtypes.FRRConfiguration, []*nadtypes.NetworkAttachmentDefinition, error) {
	...
	if len(selectedNetworks.networks) == 0 {
	...
	for nodeName, frrConfigs := range nodeToFrrConfig {
		if frrConfigs == nil {
			return nil, nil, fmt.Errorf("%w: no FRRConfiguration selected for node %q", errPending, nodeName)
		}
		...
		if ra.Spec.TargetVRF == "auto" && !matchedNetworks.HasAll(selectedNetworks.networks...) {
			return nil, nil, fmt.Errorf("%w: selected FRRConfigurations for node %q don't match all selected networks with target VRF 'auto'", errPending, nodeName)
		}
	}
