...
// updateNetworkStatus allows to report an status for networkClusterController's network via a UDN status condition
...
func (ncc *networkClusterController) updateNetworkStatus(nodeName string, handlerErr error) error {
	...
	if ncc.reportedErrorNode == nodeName && errorMsg == "" {
		...
		// zero value (aka empty string) will be returned if no more nodes have errors
		reportedErrorNode, _ = sets.New(maps.Keys(ncc.nodeErrors)...).Delete(nodeName).PopAny()
	}
