...
func (hs *HealthzServer) Run() {
	...
	go wait.Until(func() {
		logrus.Debugf("Starting goroutine for healthz on %s", hs.addr)

		listener, err := hs.listener.Listen(hs.addr)
		if err != nil {
			msg := fmt.Sprintf("Failed to start node healthz on %s: %v", hs.addr, err)
			if hs.recorder != nil {
				hs.recorder.Eventf(hs.nodeRef, v1.EventTypeWarning, "FailedToStartNodeHealthcheck", msg)
			}
			logrus.Error(msg)
			return
		}
		...
		if err := server.Serve(listener); err != nil {
			logrus.Errorf("Healthz closed with error: %v", err)
			return
		}
		logrus.Error("Unexpected healthz closed.")
