...
func (_m *AddressSetFactory) NewAddressSetOps(dbIDs *libovsdbops.DbObjectIDs, ips []net.IP) (addressset.AddressSet, []ovsdb.Operation, error) {
	ret := _m.Called(dbIDs, ips)
	...
	if rf, ok := ret.Get(1).(func(*libovsdbops.DbObjectIDs, []net.IP) []ovsdb.Operation); ok {
		r1 = rf(dbIDs, ips)
	} else {
		if ret.Get(1) != nil {
	...
	var r2 error
	if rf, ok := ret.Get(2).(func(*libovsdbops.DbObjectIDs, []net.IP) error); ok {
		r2 = rf(dbIDs, ips)
	} else {
		r2 = ret.Error(2)
	}
