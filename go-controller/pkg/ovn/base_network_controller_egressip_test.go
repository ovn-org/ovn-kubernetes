		...
		ginkgo.It("OVN network does not depend on EgressIP status for assignment", func() {
			...
			expectedDatabaseState := []libovsdbtest.TestData{
				...
				&nbdb.LogicalRouterPolicy{
					...
					ExternalIDs: getEgressIPLRPReRouteDbIDs(eIP.Name, egressPod.Namespace, egressPod.Name, IPFamilyValueV4, DefaultNetworkControllerName).GetExternalIDs(),
					UUID:        "reroute-UUID",
				},
				&nbdb.LogicalRouterPolicy{
					Priority:    types.DefaultNoRereoutePriority,
				...
				app.Action = func(ctx *cli.Context) error {
					...
					expectedDatabaseState := []libovsdbtest.TestData{
						getReRoutePolicy(egressPod.Status.PodIP, "4", "reroute-UUID", reroutePolicyNextHop,
							getEgressIPLRPReRouteDbIDs(eIP.Name, egressPod.Namespace, egressPod.Name, IPFamilyValueV4, DefaultNetworkControllerName).GetExternalIDs()),
						&nbdb.LogicalRouterPolicy{
							Priority:    types.DefaultNoRereoutePriority,
							Match:       "ip4.src == 10.128.0.0/14 && ip4.dst == 10.128.0.0/14",
							Action:      nbdb.LogicalRouterPolicyActionAllow,
				...
				app.Action = func(ctx *cli.Context) error {
					...
					if !interconnect {
						lrps = append(lrps, getReRoutePolicy(egressPod1Node1.Status.PodIP, "4", "reroute-UUID", egressPod1Node1Reroute,
							getEgressIPLRPReRouteDbIDs(eIPOVN.Name, egressPod1Node1.Namespace, egressPod1Node1.Name, IPFamilyValueV4, DefaultNetworkControllerName).GetExternalIDs()),
							getReRoutePolicy(egressPod2Node1.Status.PodIP, "4", "reroute-UUID2", egressPod2Node1Reroute,
								getEgressIPLRPReRouteDbIDs(eIPSecondaryHost.Name, egressPod2Node1.Namespace, egressPod2Node1.Name, IPFamilyValueV4, DefaultNetworkControllerName).GetExternalIDs()),
							getReRoutePolicy(egressPod3Node2.Status.PodIP, "4", "reroute-UUID3", egressPod3Node2Reroute,
								getEgressIPLRPReRouteDbIDs(eIPOVN.Name, egressPod3Node2.Namespace, egressPod3Node2.Name, IPFamilyValueV4, DefaultNetworkControllerName).GetExternalIDs()),
							getReRoutePolicy(egressPod4Node2.Status.PodIP, "4", "reroute-UUID4", egressPod4Node2Reroute,
								getEgressIPLRPReRouteDbIDs(eIPSecondaryHost.Name, egressPod4Node2.Namespace, egressPod4Node2.Name, IPFamilyValueV4, DefaultNetworkControllerName).GetExternalIDs()))
					}
				...
				app.Action = func(ctx *cli.Context) error {
					...
					if node1Zone != "remote" {
						expectedDatabaseState = append(expectedDatabaseState, getReRoutePolicy(egressPod.Status.PodIP, "4", "reroute-UUID",
							nodeLogicalRouterIPv4, getEgressIPLRPReRouteDbIDs(eIP.Name, egressPod.Namespace, egressPod.Name, IPFamilyValueV4, DefaultNetworkControllerName).GetExternalIDs()))
				...
				app.Action = func(ctx *cli.Context) error {
					...
					if !isnode2Local {
						...
						expectedDatabaseState = append(expectedDatabaseState, getReRoutePolicy(egressPod.Status.PodIP,
							...
							getEgressIPLRPReRouteDbIDs(eIP.Name, egressPod.Namespace, egressPod.Name, IPFamilyValueV6, DefaultNetworkControllerName).GetExternalIDs()))
					}
					if !isnode1Local {
			...
			app.Action = func(ctx *cli.Context) error {
				...
				expectedDatabaseState := []libovsdbtest.TestData{
					&nbdb.LogicalRouterPolicy{
						...
						ExternalIDs: getEgressIPLRPReRouteDbIDs(eIP.Name, egressPod.Namespace, egressPod.Name, IPFamilyValueV6, DefaultNetworkControllerName).GetExternalIDs(),
						UUID:        "reroute-UUID",
					},
					&nbdb.LogicalRouter{
						Name:     types.OVNClusterRouter,
			...
			app.Action = func(ctx *cli.Context) error {
				...
				expectedDatabaseState := []libovsdbtest.TestData{
					...
					&nbdb.LogicalRouterPolicy{
						...
						Options:     map[string]string{"pkt_mark": types.EgressIPNodeConnectionMark},
						ExternalIDs: getEgressIPLRPNoReRoutePodToNodeDbIDs(IPFamilyValueV6, DefaultNetworkControllerName).GetExternalIDs(),
					},
					&nbdb.LogicalRouter{
						Name: types.OVNClusterRouter,
						UUID: types.OVNClusterRouter + "-UUID",
			...
			app.Action = func(ctx *cli.Context) error {
				...
				expectedDatabaseStatewithPod := []libovsdbtest.TestData{
					...
					&nbdb.LogicalRouterPolicy{
						...
						Action:      nbdb.LogicalRouterPolicyActionReroute,
						Nexthops:    nodeLogicalRouterIPv4,
						ExternalIDs: getEgressIPLRPReRouteDbIDs(eIP.Name, egressPod1.Namespace, egressPod1.Name, IPFamilyValueV4, DefaultNetworkControllerName).GetExternalIDs(),
						UUID:        "reroute-UUID1",
					},
					&nbdb.LogicalRouter{
						Name: types.OVNClusterRouter,
				...
				err = fakeOvn.controller.addPodEgressIPAssignments(egressIPName, eIP.Status.Items, util.EgressIPMark{}, &egressPod1)
				gomega.Expect(err).NotTo(gomega.HaveOccurred())
			...
			func(interconnect bool, node1Zone, node2Zone string) {
				...
				app.Action = func(ctx *cli.Context) error {
					...
					podReRoutePolicy := &nbdb.LogicalRouterPolicy{
						...
						Action:      nbdb.LogicalRouterPolicyActionReroute,
						Nexthops:    nodeLogicalRouterIPv4,
						ExternalIDs: getEgressIPLRPReRouteDbIDs(pas.egressIPName, egressPod1.Namespace, egressPod1.Name, IPFamilyValueV4, DefaultNetworkControllerName).GetExternalIDs(),
						UUID:        "reroute-UUID1",
					}
					...
					podReRoutePolicy.ExternalIDs = getEgressIPLRPReRouteDbIDs(egressIP2Name, egressPod1.Namespace, egressPod1.Name, IPFamilyValueV4, DefaultNetworkControllerName).GetExternalIDs()
				...
				fakeOvn.startWithDBSetup(
					libovsdbtest.TestSetup{
						NBData: []libovsdbtest.TestData{
							...
							&nbdb.LogicalRouterPolicy{
								UUID:        "remove-me-UUID",
								ExternalIDs: getEgressIPLRPReRouteDbIDs(eIP.Name, egressNamespace.Name, "doesnt-exist-pod", IPFamilyValueV4, DefaultNetworkControllerName).GetExternalIDs(),
			...
			app.Action = func(ctx *cli.Context) error {
				...
				expectedDatabaseState = []libovsdbtest.TestData{
					...
					&nbdb.LogicalRouterPolicy{
						...
						Action:      nbdb.LogicalRouterPolicyActionReroute,
						Nexthops:    []string{"100.64.0.2"},
						ExternalIDs: getEgressIPLRPReRouteDbIDs(eIP.Name, egressPod2.Namespace, egressPod2.Name, IPFamilyValueV4, DefaultNetworkControllerName).GetExternalIDs(),
						UUID:        "reroute-UUID2",
					},
					&nbdb.LogicalRouter{
						Name: types.OVNClusterRouter,
				...
				expectedDatabaseState = []libovsdbtest.TestData{
					...
					&nbdb.LogicalRouterPolicy{
						...
						ExternalIDs: getEgressIPLRPReRouteDbIDs(egressIPName, egressPod1.Namespace, egressPod1.Name,
							"ip4", DefaultNetworkControllerName).GetExternalIDs(),
						UUID: "reroute-UUID1",
					},
					&nbdb.NAT{
						UUID:       "egressip-nat-UUID1",
...
func getDefaultQoSRule(isv6 bool) *nbdb.QoS {
	...
	qos := &nbdb.QoS{
		...
		ExternalIDs: getEgressIPQoSRuleDbIDs(IPFamilyValueV4, DefaultNetworkControllerName).GetExternalIDs(),
	...
	if isv6 {
		...
		qos.ExternalIDs = getEgressIPQoSRuleDbIDs(IPFamilyValueV6, DefaultNetworkControllerName).GetExternalIDs()
	}
	return qos
}
...
func getNoReRouteReplyTrafficPolicy() *nbdb.LogicalRouterPolicy {
	return &nbdb.LogicalRouterPolicy{
		...
		Action:      nbdb.LogicalRouterPolicyActionAllow,
		ExternalIDs: getEgressIPLRPNoReRouteDbIDs(types.DefaultNoRereoutePriority, ReplyTrafficNoReroute, IPFamilyValue, DefaultNetworkControllerName).GetExternalIDs(),
