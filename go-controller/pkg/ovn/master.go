	...
	opModels = []util.OperationModel{
		{
			...
			ModelPredicate: func(lrp *nbdb.LogicalRouterPort) bool { return lrp.Name == drRouterPort },
				...
				return []model.Mutation{
					{
						...
						Value:   []string{drRouterPortNamedUUID},
					},
				}
			},
			ExistingResult: &[]nbdb.LogicalRouter{},
	...
	opModels = []util.OperationModel{
		{
			...
			ModelPredicate: func(lsp *nbdb.LogicalSwitchPort) bool { return lsp.Name == drSwitchPort },
				...
				return []model.Mutation{
					{
						...
						Mutator: ovsdb.MutateOperationInsert,
						Value:   []string{drSwitchPortNamedUUID},
					},
				}
			},
			ExistingResult: &[]nbdb.LogicalSwitch{},
...
func (oc *Controller) addNodeLogicalSwitchPort(logicalSwitchName, portName, portType string, addresses []string, options map[string]string) (string, error) {
	...
	opModels := []util.OperationModel{
		{
			Model:          &logicalSwitchPort,
			ModelPredicate: func(lsp *nbdb.LogicalSwitchPort) bool { return lsp.Name == portName },
			OnModelUpdates: []interface{}{
...
func (oc *Controller) ensureNodeLogicalNetwork(node *kapi.Node, hostSubnets []*net.IPNet) error {
	...
	nodeSwToRtrUUID, err := oc.addNodeLogicalSwitchPort(nodeName, types.SwitchToRouterPrefix+nodeName,
		"router", []string{nodeLRPMAC.String()}, map[string]string{"router-port": types.RouterToSwitchPrefix + nodeName})
	if err != nil {
