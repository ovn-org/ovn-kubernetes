...
func (oc *Controller) Run(wg *sync.WaitGroup) error {
	...
	if util.DetectEndpointSlices(oc.client) {
		// Services are handled differently depending on the Kubernetes API versions
		klog.Infof("Using EndpointSlices instead of Endpoints in k8s versions > 1.19")
		...
		go func() {
			defer wg.Done()
			// use 5 workers as most of the kubernetes controllers in the controller-manager
