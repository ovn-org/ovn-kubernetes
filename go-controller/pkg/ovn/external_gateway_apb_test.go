...
var _ = ginkgo.Describe("OVN for APB External Route Operations", func() {
	...
	ginkgo.Context("on setting namespace gateway static hop", func() {
		...
		table.DescribeTable("reconciles an new pod with namespace single exgw static GW after policy is created", func(bfd bool, finalNB []libovsdbtest.TestData) {
			app.Action = func(ctx *cli.Context) error {
	...
	ginkgo.Context("hybrid route policy operations in lgw mode", func() {
		ginkgo.It("add hybrid route policy for pods", func() {
			app.Action = func(ctx *cli.Context) error {
				...
				asIndex := getHybridRouteAddrSetDbIDs("node1", fakeOvn.controller.controllerName)
				...
				dbIDs := getHybridRouteAddrSetDbIDs("node1", fakeOvn.controller.controllerName)
		...
		ginkgo.It("delete hybrid route policy for pods with force", func() {
			app.Action = func(ctx *cli.Context) error {
				...
				asIndex1 := getHybridRouteAddrSetDbIDs("node1", fakeOvn.controller.controllerName)
				as1v4, _ := addressset.GetHashNamesForAS(asIndex1)
				asIndex2 := getHybridRouteAddrSetDbIDs("node2", fakeOvn.controller.controllerName)
