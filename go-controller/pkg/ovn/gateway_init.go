...
func (oc *Controller) gatewayInit(nodeName string, clusterIPSubnet []*net.IPNet, hostSubnets []*net.IPNet,
	...
	externalSwitchPortUUID := util.GenerateNamedUUID(l3GatewayConfig.InterfaceID)
	...
	opModels = []util.OperationModel{
		{
			...
			ModelPredicate: func(lsp *nbdb.LogicalSwitchPort) bool { return lsp.Name == l3GatewayConfig.InterfaceID },
			OnModelUpdates: []interface{}{
	...
	if _, err := oc.modelClient.CreateOrUpdate(opModels...); err != nil {
		return fmt.Errorf("failed to add logical port to switch %s, err: %v", externalSwitch, err)
	}
	...
	if _, err := oc.modelClient.CreateOrUpdate(opModels...); err != nil {
		return fmt.Errorf("failed to add logical port to router %s, err: %v", gatewayRouter, err)
	}
	...
	externalLogicalSwitchPortToRouter := nbdb.LogicalSwitchPort{
		...
		Type: "router",
		Options: map[string]string{
			"router-port": externalSwitchPortToRouter,
		},
	...
	for i, nextHop := range l3GatewayConfig.NextHops {
		...
		routerStaticRoute := fmt.Sprintf("%s%s", types.GWRouterToExtSwitchPrefix, gatewayRouter)
		logicalRouterStaticRoute := nbdb.LogicalRouterStaticRoute{
			UUID:       id,
			IPPrefix:   allIPs,
			Nexthop:    nextHop.String(),
			OutputPort: []string{routerStaticRoute},
		}
		opModels = []util.OperationModel{
			{
				Model: &logicalRouterStaticRoute,
				ModelPredicate: func(lrsr *nbdb.LogicalRouterStaticRoute) bool {
					return util.SliceHasStringItem(lrsr.OutputPort, routerStaticRoute) && lrsr.Nexthop == nextHop.String()
				},
				OnModelUpdates: []interface{}{
					&logicalRouterStaticRoute.Nexthop,
				},
