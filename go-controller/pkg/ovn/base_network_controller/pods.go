...
func (bnc *BaseNetworkController) DeletePodLogicalPort(pod *kapi.Pod, portInfo *ovnutil.LPInfo,
	nadName string) (*ovnutil.LPInfo, error) {
...
func (bnc *BaseNetworkController) ReleasePodIPs(pInfo *ovnutil.LPInfo) error {
...
func (bnc *BaseNetworkController) deletePodFromNamespace(ns string, podIfAddrs []*net.IPNet, portUUID string) ([]ovsdb.Operation, error) {
	...
	if nsInfo.AddressSet != nil {
		if ops, err = nsInfo.AddressSet.DeleteIPsReturnOps(ovnutil.CreateIPAddressSlice(podIfAddrs)); err != nil {
			return nil, err
		}
	}
...
func podDeleteAllowMulticastPolicy(nbClient libovsdbclient.Client, ns string, portUUID string) error {
	return libovsdbops.DeletePortsFromPortGroup(nbClient, util.HashedPortGroup(ns), portUUID)
}
