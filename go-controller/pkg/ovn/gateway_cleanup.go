...
func (bnc *BaseNetworkController) gatewayCleanup(nodeName string) error {
	...
	bnc.staticRouteCleanup(gatewayRouter, nextHops)
	bnc.policyRouteCleanup(gatewayRouter, nextHops)
...
// TODO(dceara): do we need to move this one to base_network_controller.go?
func (bnc *BaseNetworkController) staticRouteCleanup(routerName string, nextHops []net.IP) {
	ips := sets.Set[string]{}
	...
	err := libovsdbops.DeleteLogicalRouterStaticRoutesWithPredicate(bnc.nbClient, routerName, p)
	if err != nil && !errors.Is(err, libovsdbclient.ErrNotFound) {
...
// - if the LRP exists and has the len(nexthops) == 1: it removes
// the LRP completely
func (bnc *BaseNetworkController) policyRouteCleanup(routerName string, nextHops []net.IP) {
	for _, nextHop := range nextHops {
		gwIP := nextHop.String()
		...
		err := libovsdbops.DeleteNextHopFromLogicalRouterPoliciesWithPredicate(bnc.nbClient, routerName, policyPred, gwIP)
