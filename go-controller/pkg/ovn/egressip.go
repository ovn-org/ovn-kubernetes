...
func (oc *DefaultNetworkController) addPodEgressIPAssignments(name string, statusAssignments []egressipv1.EgressIPStatusItem, pod *kapi.Pod) error {
	...
	// TODO(tssurya): Check if this loop also needs to be locked for nodeZoneState key?
	proceed := false
...
func (oc *DefaultNetworkController) deleteEgressIPAssignments(name string, statusesToRemove []egressipv1.EgressIPStatusItem) error {
	...
	for _, statusToRemove := range statusesToRemove {
		...
		for podKey, podStatus := range oc.eIPC.podAssignment {
			...
			// TODO(tssurya): Would be nice to lock the address-set deletion as well but its not that important
...
func (oc *DefaultNetworkController) addEgressNode(node *v1.Node) error {
	...
	// NOTE: We still need to take care of adding reroute policies when remote egress
	// nodes are added. Hence we need to call reconcile even if node that got added was a remote one.
...
func (oc *DefaultNetworkController) deleteEgressNode(node *v1.Node) error {
	...
	if oc.isLocalZoneNode(node) {
		...
		if errors.Is(err, libovsdbclient.ErrNotFound) {
			...
			errorAggregate = append(errorAggregate, fmt.Errorf("unable to remove GARP configuration on external logical switch port for egress node: %s, err: %v", node.Name, err))
		}
	}
	// NOTE: We still need to take care of deleting reroute policies when remote egress
	// nodes are deleting. Hence we need to call reconcile even if node that got deleted was a remote one.
	...
	for _, egressIP := range egressIPs.Items {
		for _, status := range egressIP.Status.Items {
			if status.Node == node.Name {
				...
				if err := oc.reconcileEgressIP(nil, &egressIP); err != nil {
					errorAggregate = append(errorAggregate, fmt.Errorf("re-assignment for EgressIP: %s failed, unable to update object, err: %w", egressIP.Name, err))
				}
				break
			}
		}
...
// reconcileEgressIPNode with respect and old and new status of a node
func (oc *DefaultNetworkController) reconcileEgressIPNode(oldNode, newNode *v1.Node) error {
	...
	var isOldReady, isNewReady bool
	var nodeName string
	if oldNode != nil {
...
func (e *egressIPZoneController) deleteEgressIPStatusSetup(name string, status egressipv1.EgressIPStatusItem) ([]net.IP, error) {
	...
	if loadedEgressNode && isLocalZoneEgressNode {
		...
		natPred := func(nat *nbdb.NAT) bool {
			// TODO(tssurya): investigate if we should be deleting NATs only from status.Node
