...
func (oc *Controller) createDefaultNoReroutePodPolicies(v4ClusterSubnet, v6ClusterSubnet []*net.IPNet) {
	for i, v4Subnet := range v4ClusterSubnet {
		match := fmt.Sprintf("ip4.src == %s && ip4.dst == %s", v4Subnet.String(), v4Subnet.String())
		if err := oc.createLogicalRouterPolicy(fmt.Sprintf("v4policy%v", i), match, types.DefaultNoRereoutePriority); err != nil {
			klog.Errorf("Unable to create IPv4 no-reroute pod policies, err: %v", err)
		}
	}
	for i, v6Subnet := range v6ClusterSubnet {
		match := fmt.Sprintf("ip6.src == %s && ip6.dst == %s", v6Subnet.String(), v6Subnet.String())
		if err := oc.createLogicalRouterPolicy(fmt.Sprintf("v6policy%v", i), match, types.DefaultNoRereoutePriority); err != nil {
...
func (oc *Controller) createDefaultNoRerouteNodePolicies(v4NodeAddr, v6NodeAddr net.IP, v4ClusterSubnet, v6ClusterSubnet []*net.IPNet) error {
	if v4NodeAddr != nil {
		for i, v4Subnet := range v4ClusterSubnet {
			...
			if err := oc.createLogicalRouterPolicy(fmt.Sprintf("v4policyNode%v", i), match, types.DefaultNoRereoutePriority); err != nil {
	...
	if v6NodeAddr != nil {
		for i, v6Subnet := range v6ClusterSubnet {
			...
			if err := oc.createLogicalRouterPolicy(fmt.Sprintf("v6policyNode%v", i), match, types.DefaultNoRereoutePriority); err != nil {
...
func (oc *Controller) createLogicalRouterPolicy(namedUUID, match string, priority int) error {
	logicalRouter := nbdb.LogicalRouter{}
	logicalRouterPolicy := nbdb.LogicalRouterPolicy{
		UUID:     namedUUID,
		Priority: priority,
