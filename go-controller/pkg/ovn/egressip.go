...
func (oc *DefaultNetworkController) isEgressNodeReachable(egressNode *kapi.Node) bool {
	...
	if eNode, exists := oc.eIPC.allocator.cache[egressNode.Name]; exists {
		return eNode.isReachable || nodereachability.IsReachable(eNode.name, eNode.mgmtIPs, eNode.healthClient)
	}
	return false
}
...
func (oc *DefaultNetworkController) initClusterEgressPolicies(nodes []interface{}) error {
	...
	go nodereachability.CheckNodesReachability(oc.stopChan, oc.checkEgressNodesReachabilityIterate)
	return nil
}
...
func (oc *DefaultNetworkController) checkEgressNodesReachabilityIterate() {
	reAddOrDelete := map[string]bool{}
	oc.eIPC.allocator.Lock()
	for _, eNode := range oc.eIPC.allocator.cache {
		if eNode.isEgressAssignable && eNode.isReady {
			wasReachable := eNode.isReachable
			isReachable := nodereachability.IsReachable(eNode.name, eNode.mgmtIPs, eNode.healthClient)
