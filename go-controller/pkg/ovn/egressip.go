	...
	for _, namespace := range namespaces {
		...
		for _, node := range nodes {
			...
			if localZoneNodes.Has(node.Name) {
				if e.v4 {
					if gatewayRouterIP, err := e.getTopologyScopedNextHop(ni, node.Name, false); err != nil {
				...
				if e.v6 {
					if gatewayRouterIP, err := e.getTopologyScopedNextHop(ni, node.Name, true); err != nil {
...
// getTopologyScopedNextHop determines the next hop for a given Node considering the network topology type
func (e *EgressIPController) getTopologyScopedNextHop(ni util.NetInfo, nodeName string, wantsIPv6 bool) (net.IP, error) {
	if ni.TopologyType() == types.Layer3Topology {
		// fetch gateway router IP for port attached to join switch
		return e.getRouterPortIP(types.GWRouterToJoinSwitchPrefix+ni.GetNetworkScopedGWRouterName(nodeName), wantsIPv6)
	}
	if ni.TopologyType() == types.Layer2Topology {
		// For L2, if the pod selected by an EgressIP and also that EgressIP is managed by this controller, then the next hop must be
		// retrieved from the node annotation. If a node is managed by another controller, we must get the remote Node gateway router to switch port IP
		...
		return e.getGatewayRouterJoinIPFromAnnotations(ni.GetNetworkName(), node, wantsIPv6)
	}
	return nil, fmt.Errorf("unsupported network topology %s", ni.TopologyType())
}

func (e *EgressIPController) getGatewayRouterJoinIPFromAnnotations(networkName string, node *kapi.Node, wantsIPv6 bool) (net.IP, error) {
	if wantsIPv6 {
		return util.ParseNodeGatewayRouterJoinIPv6(node, networkName)
	}
	return util.ParseNodeGatewayRouterJoinIPv4(node, networkName)
}
...
func (e *EgressIPController) getNextHop(ni util.NetInfo, egressNodeName, egressIP, egressIPName string, isLocalZoneEgressNode, isOVNNetwork bool) (string, error) {
	...
	if isLocalZoneEgressNode || ni.TopologyType() == types.Layer2Topology {
		if isOVNNetwork {
			gatewayRouterIP, err := e.getTopologyScopedNextHop(ni, egressNodeName, isEgressIPv6)
...
func (e *EgressIPController) createReroutePolicyOps(ni util.NetInfo, ops []ovsdb.Operation, podIPNets []*net.IPNet, status egressipv1.EgressIPStatusItem,
	...
	for _, podIPNet := range util.MatchAllIPNetFamily(isEgressIPv6, podIPNets) {
		...
		routerName, err := getTopologyScopedRouterName(ni, podNodeName)
		if err != nil {
			return ops, err
		}
