...
func (oc *DefaultNetworkController) addPodExternalGWForNamespace(namespace string, pod *kapi.Pod, egress gatewayInfo) error {
	...
	// Include all the dynamic Hop gateway IPs from the policies that apply to this namespace
	tmpIPs, err := oc.apbExternalRouteController.GetDynamicGatewayIPsForTargetNamespace(namespace)
	if err != nil {
		return err
	}
	existingGWs.Insert(tmpIPs.UnsortedList()...)
