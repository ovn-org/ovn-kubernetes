...
func (n kNetworkPolicy) getDefaultDenyData(networkPolicy *knet.NetworkPolicy, ports []string, logSeverity nbdb.ACLSeverity) []libovsdbtest.TestData {
...
func (n kNetworkPolicy) getPolicyData(networkPolicy *knet.NetworkPolicy, policyPorts []string, peerNamespaces []string, tcpPeerPorts []int32, logSeverity nbdb.ACLSeverity) []libovsdbtest.TestData {
...
func (n kNetworkPolicy) generateExpectedACLDataForFirstPolicyOnNamespace(policy knet.NetworkPolicy, loggingSeverity nbdb.ACLSeverity) []libovsdbtest.TestData {
...
func (p multicastPolicy) getMulticastPolicyExpectedData(ns string, ports []string) []libovsdbtest.TestData {
		...
		for _, m := range getIpModes() {
			...
			ginkgo.It("tests enabling/disabling multicast in a namespace "+ipModeStr(m), func() {
				app.Action = func(ctx *cli.Context) error {
					...
					gomega.Expect(err).NotTo(gomega.HaveOccurred())
					gomega.Eventually(fakeOvn.testHarness.NBClient).Should(libovsdbtest.HaveData(expectedData...))
					...
					gomega.Eventually(fakeOvn.testHarness.NBClient).Should(libovsdbtest.HaveData(acls))
					return nil
				}
			...
			ginkgo.It("tests adding a pod to a multicast enabled namespace "+ipModeStr(m), func() {
				app.Action = func(ctx *cli.Context) error {
					...
					_, err = fakeOvn.fakeClient.KubeClient.CoreV1().Namespaces().Update(context.TODO(), ns, metav1.UpdateOptions{})
					gomega.Expect(err).NotTo(gomega.HaveOccurred())
					gomega.Eventually(fakeOvn.testHarness.NBClient).Should(libovsdbtest.HaveData(append(expectedData, &nbdb.LogicalSwitch{
					...
					gomega.Eventually(fakeOvn.testHarness.NBClient).Should(libovsdbtest.HaveData(expectedDataWithPods...))
	...
	ginkgo.Context("during execution", func() {
		...
		ginkgo.It("correctly creates and deletes a networkpolicy allowing a port to a local pod", func() {
			app.Action = func(ctx *cli.Context) error {
				...
				gomega.Eventually(fakeOvn.testHarness.NBClient).Should(libovsdbtest.HaveData(expectedDataWithPolicy2...))
				...
				gomega.Eventually(fakeOvn.testHarness.NBClient).Should(libovsdbtest.HaveData(expectedDataWithoutPolicy2...))
				...
				gomega.Eventually(fakeOvn.testHarness.NBClient).Should(libovsdbtest.HaveData(append(acls, getExpectedDataPodsAndSwitches([]testPod{nPodTest}, []string{"node1"})...)))
		...
		ginkgo.Context("ACL logging for network policies", func() {
			...
			table.DescribeTable("ACL logging for network policies reacts to severity updates", func(networkPolicies ...knet.NetworkPolicy) {
				...
				app.Action = func(ctx *cli.Context) error {
					...
					gomega.Eventually(fakeOvn.testHarness.NBClient).Should(libovsdbtest.HaveData(initialExpectedData...))

					var provisionedPolicies []libovsdbtest.TestData
					...
					gomega.Eventually(fakeOvn.testHarness.NBClient).Should(libovsdbtest.HaveData(append(initialExpectedData, provisionedPolicies...)))
					...
					gomega.Eventually(fakeOvn.testHarness.NBClient).Should(libovsdbtest.HaveData(expectedDataAfterLoggingSeverityUpdate...))
					return nil
				}
...
var _ = ginkgo.Describe("OVN NetworkPolicy Low-Level Operations", func() {
	...
	ginkgo.It("computes match strings from address sets correctly", func() {
		...
		gomega.Expect(actual).To(gomega.ConsistOf(libovsdbtest.EqualIgnoringUUIDs(expected)))
