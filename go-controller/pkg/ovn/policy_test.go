...
func addExpectedUpdateNetworkPolicyACLLoggingSeverity(fExec *ovntest.FakeExec, logSeverity string, policies ...knet.NetworkPolicy) {
	aclNames := composeACLNamesFromMultiplePolicies(policies...)
	aclUUIDs := generateACLUUIDs(aclNames...)

	for i, aclName := range aclNames {
		fExec.AddFakeCmd(&ovntest.ExpectedCmd{
			Cmd:    fmt.Sprintf("ovn-nbctl --timeout=15 --format=csv --data=bare --no-heading --columns=_uuid find ACL name=%s", aclName),
			Output: aclUUIDs[i],
		})
...
func composeACLNamesFromMultiplePolicies(policies ...knet.NetworkPolicy) []string {
