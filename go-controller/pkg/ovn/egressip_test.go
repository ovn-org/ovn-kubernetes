		...
		ginkgo.DescribeTable(
			...
			func(
				...
				app.Action = func(ctx *cli.Context) error {
					...
					uuids, moreTestData := buildDefaultNoRerouteNodePolicies([]string{podV4IP, podV6IP}, []string{node1IPv4, "fc00:f853:ccd:e793::13"}, nil, types.DefaultNetworkName, fakeOvn.controller.eIPC.controllerName)
					ovnClusterRouterPolicies = append(ovnClusterRouterPolicies, uuids...)
					expectedDatabaseStatewithPod := []libovsdbtest.TestData{
						getNoReRouteReplyTrafficPolicy(types.DefaultNetworkName, fakeOvn.controller.eIPC.controllerName),
						podEIPSNAT,
...
func buildDefaultNoRerouteNodePolicies(podAddresses, nodeAddresses, serviceAddresses []string, network, controller string) ([]string, []libovsdbtest.TestData) {
	testData := []libovsdbtest.TestData{}
	uuids := []string{}

	egressIPServedPodsASv4, egressIPServedPodsASv6 := buildEgressIPServedPodsAddressSets(podAddresses, network, controller)
	egressNodeIPsASv4, egressNodeIPsASv6 := buildEgressIPNodeAddressSets(nodeAddresses)
	egressSVCServedPodsASv4, egressSVCServedPodsASv6 := buildEgressServiceAddressSets(serviceAddresses)
