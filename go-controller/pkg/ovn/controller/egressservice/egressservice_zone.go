...
func (c *Controller) repair() error {
	...
	if err := c.setPodIPsInAddressSet(createIPAddressSetSlice(allLocalV4Eps, allLocalV6Eps)); err != nil {
...
func (c *Controller) syncEgressService(key string) error {
	...
	createOps, err = c.addPodIPsToAddressSetOps(createIPAddressSetSlice(v4LocalToAdd, v6LocalToAdd))
	if err != nil {
		return err
	}
	allOps = append(allOps, createOps...)
	...
	deleteOps, err = c.deletePodIPsFromAddressSetOps(createIPAddressSetSlice(v4LocalToRemove, v6LocalToRemove))
	if err != nil {
		return err
	}
	allOps = append(allOps, deleteOps...)
...
func (c *Controller) clearServiceResourcesAndRequeue(key string, svcState *svcState) error {
	...
	delAddrSetOps, err := c.deletePodIPsFromAddressSetOps(createIPAddressSetSlice(svcState.v4LocalEndpoints.UnsortedList(), svcState.v6LocalEndpoints.UnsortedList()))
	if err != nil {
		return err
	}
