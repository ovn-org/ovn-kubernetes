...
func (m *externalPolicyManager) applyPodGWPolicy(pod *v1.Pod, externalRoutePolicy *adminpolicybasedrouteapi.AdminPolicyBasedExternalRoute) error {
	...
	routePolicies, err := m.getRoutePoliciesForPodGateway(pod, externalRoutePolicy)
	if err != nil {
		return err
	}
	// update all namespaces targeted by this pod's policy to include the new pod IP as their external GW
	err = m.applyProcessedPolicy(externalRoutePolicy.Name, routePolicies)
	if err != nil {
		return err
	}
	gwInfoMap, err := m.agregateDynamicRouteGatewayInformation(pod, routePolicies)
	if err != nil {
		return err
	}
...
func (m *externalPolicyManager) processUpdatePod(updatedPod *v1.Pod, oldTargetNs sets.Set[string]) error {
	...
	for ns := range nsToRemove {
		...
		cacheInfo, found := m.getNamespaceInfoFromCache(ns)
		if !found {
			klog.Warningf("Attempting to remove pod gateway %s/%s from a namespace that does not exist %s", updatedPod.Namespace, updatedPod.Name, ns)
			continue
		}

		gateways, found := cacheInfo.dynamicGateways[key]
		if !found {
			klog.Warningf("Pod %s/%s not found in dynamic cacheInfo for namespace %s", updatedPod.Namespace, updatedPod.Name, ns)
			m.unlockNamespaceInfoCache(ns)
			continue
		}
		annotatedGWIPs, err := m.calculateAnnotatedNamespaceGatewayIPsForNamespace(ns)
		if err != nil {
			return err
		}
		coexistingIPs, err := m.retrieveCoexistingDynamicGatewayIPsToPoliciesInNamespace(ns, cacheInfo.policies)
		if err != nil {
			return err
		}
	...
	for ns := range nsToAdd {
		// the pod's gatewayInfo is unique to a namespace, so we retrieve the correct one
		// for the given target namespace from the pre-processed policies. It uses
		// the target namespace and the key (pod_namespace,pod_name) as keys.
		gatewayInfo, err := m.findGatewayInfoForPodInTargetNamespace(key, ns, pp)
		if err != nil {
			return err
		}
		// use the pod's gatewayInfo to update the logical routes for all the pod's in the target namespace
		err = m.addGWRoutesForNamespace(ns, gatewayInfoList{gatewayInfo})
		if err != nil {
			return err
		}
		cacheInfo, found := m.getNamespaceInfoFromCache(ns)
		if !found {
			cacheInfo = m.newNamespaceInfoInCache(ns)
		}
		// add pod gateway information to the namespace cache
		cacheInfo.dynamicGateways[key] = gatewayInfo
		m.unlockNamespaceInfoCache(ns)
	}
...
func (m *externalPolicyManager) findGatewayInfoForPodInTargetNamespace(key ktypes.NamespacedName, targetNamespace string, processedPolicies []*routePolicy) (*gatewayInfo, error) {
	for _, p := range processedPolicies {
		namespaces, err := m.listNamespacesBySelector(p.labelSelector)
		if err != nil {
			return nil, err
		}
...
func (m *externalPolicyManager) deletePodGatewayInNamespace(pod *v1.Pod, targetNamespace string) error {
	...
	coexistingIPs, err := m.retrieveCoexistingDynamicGatewayIPsToPoliciesInNamespace(targetNamespace, cacheInfo.policies)
	if err != nil {
		return err
	}
...
// getRoutePoliciesForPodGateway iterates through the dynamic hops of a given external route policy spec to determine the pod's GW information.
...
func (m *externalPolicyManager) getRoutePoliciesForPodGateway(newPod *v1.Pod, externalRoutePolicy *adminpolicybasedrouteapi.AdminPolicyBasedExternalRoute) (*routePolicy, error) {
