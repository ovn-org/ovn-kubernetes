...
var _ = Describe("OVN External Gateway policy", func() {
	...
	var _ = Context("when updating a policy", func() {
		...
		It("validates that BFD Rate limiter decreases proportionally to number of BFD connections", func() {
			staticRoutes := []string{"10.10.10.1"}
			staticMultiIPPolicy := newPolicy("multiIPPolicy",
				&v1.LabelSelector{MatchLabels: targetNamespace1Match},
				sets.New("10.10.10.1"),
			...
			Expect(err).NotTo(HaveOccurred())
			Expect(len(bfdBands)).To(Equal(1))
			Expect(bfdBands[0].Rate).To(Equal(70)) // rate is 10 plus existing BFD connections(60)

			// decrease the number of bfd connections to 60
			p, err := fakeRouteClient.K8sV1().AdminPolicyBasedExternalRoutes().Get(context.TODO(), staticMultiIPPolicy.Name, v1.GetOptions{})
			Expect(err).NotTo(HaveOccurred())
			p.Spec.NextHops.StaticHops = p.Spec.NextHops.StaticHops[:60]
			staticRoutes = staticRoutes[:60]
			p.Generation++
			_, err = fakeRouteClient.K8sV1().AdminPolicyBasedExternalRoutes().Update(context.Background(), p, v1.UpdateOptions{})
			Expect(err).NotTo(HaveOccurred())
			...
			Expect(err).NotTo(HaveOccurred())
			Expect(len(bfdBands)).To(Equal(1))
			Expect(bfdBands[0].Rate).To(Equal(70)) // number is still only 10 less than cap so no decrease observed

			// decrease the number of bfd connections to 59
			p, err = fakeRouteClient.K8sV1().AdminPolicyBasedExternalRoutes().Get(context.TODO(), staticMultiIPPolicy.Name, v1.GetOptions{})
			Expect(err).NotTo(HaveOccurred())
			p.Spec.NextHops.StaticHops = p.Spec.NextHops.StaticHops[:59]
			staticRoutes = staticRoutes[:59]
			p.Generation++
			_, err = fakeRouteClient.K8sV1().AdminPolicyBasedExternalRoutes().Update(context.Background(), p, v1.UpdateOptions{})
			Expect(err).NotTo(HaveOccurred())
			...
			Expect(err).NotTo(HaveOccurred())
			Expect(len(bfdBands)).To(Equal(1))
			Expect(bfdBands[0].Rate).To(Equal(69)) // decreases it to 10 more than existing bfds
		})
