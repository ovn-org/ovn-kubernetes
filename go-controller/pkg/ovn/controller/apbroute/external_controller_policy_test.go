...
func listRoutePolicyInCache() []*adminpolicybasedrouteapi.AdminPolicyBasedExternalRoute {
	rp, err := externalController.routeLister.List(labels.Everything())
	Expect(err).NotTo(HaveOccurred())
	match := make([]*adminpolicybasedrouteapi.AdminPolicyBasedExternalRoute, 0)
	for _, policy := range rp {
		if _, found := externalController.mgr.getRoutePolicyFromCache(policy.Name); found {
			match = append(match, policy)
		}
	}
	return match
}
...
var _ = Describe("OVN External Gateway policy", func() {
	...
	var _ = Context("When adding new policies", func() {
		...
		It("registers the new policy with multiple namespace matching", func() {
			...
			Eventually(func() []*adminpolicybasedrouteapi.AdminPolicyBasedExternalRoute {
				return listRoutePolicyInCache()
			}, 5).Should(HaveLen(1))
			Eventually(func(g Gomega) {
				p, found := externalController.mgr.getRoutePolicyFromCache(multipleMatchPolicy.Name)
		...
		It("registers a second policy with no overlaping IPs", func() {
			...
			Eventually(func() []*adminpolicybasedrouteapi.AdminPolicyBasedExternalRoute { return listRoutePolicyInCache() }, 5).
				Should(HaveLen(2))
			Eventually(listNamespaceInfo(), 5).Should(HaveLen(1))
			Eventually(func() *namespaceInfo { return getNamespaceInfo(namespaceTest.Name) }, 5).Should(
				BeEquivalentTo(
	...
	var _ = Context("when deleting a policy", func() {
		...
		It("validates that the IPs of the policy are no longer reflected on the targeted namespaces when the policy is deleted an no other policy overlaps", func() {
			...
			Eventually(func() []*adminpolicybasedrouteapi.AdminPolicyBasedExternalRoute { return listRoutePolicyInCache() }, 5).Should(HaveLen(2))
			Eventually(func() []*namespaceInfo { return listNamespaceInfo() }, 5).Should(HaveLen(1))
		...
		It("validates that the IPs of a deleted policy won't show up in a non-matching namespace after the policy is deleted", func() {
			...
			Eventually(func() []*adminpolicybasedrouteapi.AdminPolicyBasedExternalRoute { return listRoutePolicyInCache() }, 5).Should(HaveLen(2))
			Eventually(func() []*namespaceInfo { return listNamespaceInfo() }, 5).Should(HaveLen(0))
		...
		It("validates that an overlapping IP from another policy will not be deleted when one of the overlaping policies is deleted", func() {
			...
			Eventually(func() []*adminpolicybasedrouteapi.AdminPolicyBasedExternalRoute { return listRoutePolicyInCache() }, 5).Should(HaveLen(4))
			deletePolicy(staticPolicy.Name, fakeRouteClient)
			deletePolicy(dynamicPolicy.Name, fakeRouteClient)
			Eventually(func() []*adminpolicybasedrouteapi.AdminPolicyBasedExternalRoute { return listRoutePolicyInCache() }, 5).Should(HaveLen(2))
			Eventually(func() *namespaceInfo {
