...
func listNamespaceInfo() []*namespaceInfo {
	nsList, err := fakeClient.CoreV1().Namespaces().List(context.Background(), v1.ListOptions{})
	Expect(err).NotTo(HaveOccurred())
	ret := make([]*namespaceInfo, 0)
	for _, ns := range nsList.Items {
		nsInfo, found := mgr.getNamespaceInfoFromCache(ns.Name)
		if found {
			mgr.unlockNamespaceInfoCache(ns.Name)
			ret = append(ret, nsInfo)
		}
	}
	return ret
}
...
func deepCopyNamespaceInfo(source, destination *namespaceInfo) {
	...
	destination.staticGateways = gatewayInfoList.Insert(source.staticGateways)
...
var _ = Describe("OVN External Gateway namespace", func() {
	...
	var _ = Context("When no pod or namespace routing network annotations coexist with the policies", func() {
		...
		var _ = Context("When creating new namespaces", func() {
			...
			It("registers the new namespace with no matching policies", func() {
				...
				Eventually(externalController.mgr.routePolicyCache[dynamicPolicy.Name].Spec, 5).Should(Equal(dynamicPolicy.Spec))
			...
			It("registers the new namespace with one matching policy containing one static gateway", func() {
				...
				Eventually(func() []*adminpolicybasedrouteapi.AdminPolicyBasedExternalRoute { return listRoutePolicyInCache() }, 5).Should(HaveLen(1))
				Eventually(externalController.mgr.routePolicyCache[staticPolicy.Name].Spec, 5).Should(Equal(staticPolicy.Spec))
	...
	var _ = Context("When deleting a namespace", func() {
		...
		It("validates that the namespace cache is empty and marked as deleted when the namespace was a recipient for policies", func() {
			...
			Expect(externalController.mgr.namespaceLockMap).To(HaveLen(0))
			Eventually(func() []*namespaceInfo { return listNamespaceInfo() }, 5).Should(HaveLen(1))
			Eventually(func() *namespaceInfo { return getNamespaceInfo(namespaceTest.Name) }, 5).Should(
				Equal(
					&namespaceInfo{
	...
	var _ = Context("When updating an existing namespace", func() {
		...
		It("validates that a namespace is targeted by an existing policy after its labels are updated to match the policy's label selector", func() {
			...
			Eventually(func() []*namespaceInfo { return listNamespaceInfo() }, 15).Should(HaveLen(0))
			updateNamespaceLabel(namespaceTest2.Name, staticPolicy.Spec.From.NamespaceSelector.MatchLabels, fakeClient)
			Eventually(func() []*namespaceInfo { return listNamespaceInfo() }, 15).Should(HaveLen(1))
