...
func (c *Controller) setPodForNQOS(pod *v1.Pod, nqosState *networkQoSState, namespace *v1.Namespace) error {
	...
	if err != nil {
		if apierrors.IsNotFound(err) {
			klog.Warningf("Node %s not found, not retrying", pod.Spec.NodeName)
			return nil
		}
	...
	if err == nil && len(addresses) == 0 {
		...
		if strings.Contains(err.Error(), "no ovn pod annotation for network") {
			klog.V(6).Infof("Pod %s/%s has no IPs for network %s", pod.Namespace, pod.Name, nqosState.networkAttachmentName)
			return nil
		}
	...
	if c.isNodeInLocalZone(node) {
		...
		klog.V(4).Infof("Node %s not in local zone, skip pod %s", node.Name, fullPodName)
	}
