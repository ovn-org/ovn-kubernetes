...
func NewController(
	...
	c := &Controller{
		...
		zone:                      zone,
		nqosCache:                 make(map[string]*networkQoSState),
	}

	klog.V(5).Info("Setting up event handlers for Network QoS")
	...
	_, err = nodeInformer.Informer().AddEventHandler(factory.WithUpdateHandlingForObjReplace(cache.ResourceEventHandlerFuncs{
		AddFunc:    c.onNQOSNodeAdd,
		UpdateFunc: c.onNQOSNodeUpdate,
		DeleteFunc: c.onNQOSNodeDelete,
	}))
	if err != nil {
...
func (c *Controller) Run(threadiness int, stopCh <-chan struct{}) {
	...
	if !util.WaitForInformerCacheSyncWithTimeout(c.controllerName, stopCh, c.nqosCacheSynced, c.nqosNamespaceSynced, c.nqosPodSynced) {
		utilruntime.HandleError(fmt.Errorf("timed out waiting for network qos caches to sync"))
...
// onNQOSNodeAdd queues the node for processing.
func (c *Controller) onNQOSNodeAdd(obj interface{}) {
	key, err := cache.MetaNamespaceKeyFunc(obj)
	if err != nil {
		utilruntime.HandleError(fmt.Errorf("couldn't get key for object %+v: %v", obj, err))
		return
	}
	klog.V(5).Infof("Adding Node in Network QoS controller %s", key)
...
func (c *Controller) onNQOSNodeUpdate(oldObj, newObj interface{}) {
	...
	key, err := cache.MetaNamespaceKeyFunc(newObj)
	if err == nil {
		klog.V(5).Infof("Updating Node in Network QoS controller %s: "+
			"nodeLabels %v, isHostCIDRsAltered?: %v", key, newNodeLabels, isHostCIDRsAltered)
		c.nqosNodeQueue.Add(key)
	}
}

// onNQOSNodeDelete queues the node for processing.
func (c *Controller) onNQOSNodeDelete(obj interface{}) {
	key, err := cache.MetaNamespaceKeyFunc(obj)
	if err != nil {
		utilruntime.HandleError(fmt.Errorf("couldn't get key for object %+v: %v", obj, err))
		return
	}
	klog.V(5).Infof("Deleting Node Network QoS %s", key)
	c.nqosNodeQueue.Add(key)
}
