...
var _ = Describe("NetworkQoS Controller", func() {
	var _ = Context("When starting controller with NetworkQoS, Pod and Node objects", func() {
		It("creates address sets for source and destination pod selectors", func() {
		...
		It("creates QoS rules in ovn nb", func() {
		...
		It("adds IP to destination address set for matching pod", func() {
		...
		It("updates match strings if egress rules change", func() {
			...
			//Expect(qos1.Match).Should(Equal(fmt.Sprintf("ip4.src == $%s && (ip4.dst == $%s || ip4.dst == 128.118.0.0/17) && udp && udp.dst == 9090", srcHashName4, dst3HashName4)))
		})

		It("removes IP from destination address set if pod's labels don't match the selector", func() {
		...
		It("adds IP to destination address set again if pod's labels match the selector", func() {
			updatePod := app1Pod.DeepCopy()
		...
		It("removes IP from destination address set if target namespace labels don't match the selector", func() {
		...
		It("adds IP to destination address set again if namespace's labels match the selector", func() {
			ns, err := fakeKubeClient.CoreV1().Namespaces().Get(context.TODO(), app1Namespace, metav1.GetOptions{})
			Expect(err).NotTo(HaveOccurred())
			ns.ResourceVersion = time.Now().String()
			ns.Labels["app"] = "app1"
		...
		It("removes IP from destination address set if namespace selector changes", func() {
			nqosUpdate, err := fakeNQoSClient.K8sV1().NetworkQoSes(nqosNamespace).Get(context.TODO(), nqosName, metav1.GetOptions{})
			Expect(err).NotTo(HaveOccurred())
		...
		It("adds IP to destination address set if namespace selector is restored", func() {
			nqosUpdate, err := fakeNQoSClient.K8sV1().NetworkQoSes(nqosNamespace).Get(context.TODO(), nqosName, metav1.GetOptions{})
			Expect(err).NotTo(HaveOccurred())
			nqosUpdate.Spec.Egress[0].Classifier.To[0].NamespaceSelector = &metav1.LabelSelector{
				MatchLabels: map[string]string{
		...
		It("adds IP to destination address set of the second rule for matching pod", func() {
		...
		It("adds new QoS rule to ovn nb when a new Egress rule is added", func() {
			nqosUpdate, err := fakeNQoSClient.K8sV1().NetworkQoSes(nqosNamespace).Get(context.TODO(), nqosName, metav1.GetOptions{})
			Expect(err).NotTo(HaveOccurred())
		...
		It("will not handle NetworkQos with unknown NetworkAttachmentDefinition in spec", func() {
		...
		It("handles NetworkQos on secondary network", func() {
		...
		It("uses namespace's address set as source if pod selector is not provided in source", func() {
		...
		It("removes IP from destination address set if pod is deleted", func() {
			err := fakeKubeClient.CoreV1().Pods(app1Pod.Namespace).Delete(context.TODO(), app1Pod.Name, metav1.DeleteOptions{})
			Expect(err).NotTo(HaveOccurred())
			eventuallyAddressSetHasNo(defaultAddrsetFactory, nqosNamespace, nqosName, "0", "0", defaultControllerName, "10.194.188.4")
		})

		It("removes IP from destination address set of the second rule if namespace is deleted", func() {
		...
		It("deletes stale QoS from ovn nb when Egress rule is deleted", func() {
		...
		It("unbinds QoS rule from logical switch when no source pods is selected", func() {
		...
		It("deletes QoS after NetworkQoS object is deleted", func() {
		...
		It("generates correct logical switch name for localnet topology", func() {
		...
		It("generates correct logical switch name for layer2 topology", func() {
