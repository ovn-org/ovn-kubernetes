...
func (nqosState *networkQoSState) initAddressSets(addressSetFactory addressset.AddressSetFactory, controllerName string) error {
	...
	for ruleIndex, rule := range nqosState.EgressRules {
		for destIndex, dest := range rule.Classifier.Destinations {
			...
			dest.DestAddrSet, err = ensureAddressSet(addressSetFactory,
				GetNetworkQoSAddrSetDbIDs(nqosState.namespace, nqosState.name, strconv.Itoa(ruleIndex), strconv.Itoa(destIndex), controllerName))
			if err != nil {
...
func (nqosState *networkQoSState) configureSourcePod(ctrl *Controller, pod *v1.Pod, addresses []string) error {
	...
	if switchName == "" {
		return fmt.Errorf("failed to get logical switch name for node %s", pod.Spec.NodeName)
	}
...
func (nqosState *networkQoSState) removePodFromSource(ctrl *Controller, fullPodName string, addresses []string) error {
	if len(addresses) == 0 {
		// if no addresses is supplied, try lookup in cache
	...
	zeroedNodes := []string{}
	// go through nodes cache to remove the pod, since node is unknown in pod deletion path
	nqosState.NodeRefs.Range(func(key, val any) bool {
		...
		if len(podList) == 0 {
			zeroQoSSwitches = append(zeroQoSSwitches, ctrl.getLogicalSwitchName(nodeName))
			zeroedNodes = append(zeroedNodes, nodeName)
		} else {
			nqosState.NodeRefs.Store(nodeName, podList)
		}
		return true
	})
	// unbind qos from nodes which doesn't have source pods any more
	start := time.Now()
...
func (dest *Destination) addPod(podNamespace, podName string, addresses []string) error {
	...
	dest.Namespaces.Store(podNamespace, true)
	return nil
}
