...
type nodeState struct {
	...
	// transit switch IPs, only used when IC is enabled
	transitIPV4 net.IP
	transitIPV6 net.IP
}
...
func (c *Controller) repair() error {
	...
	lrpPredicate := func(item *nbdb.LogicalRouterPolicy) bool {
		...
		if config.OVNKubernetesFeature.EnableInterconnect {
			zoneKnown, svcNodeInLocalZone := c.nodeZoneState(node.name)
			if !zoneKnown {
	...
	if config.OVNKubernetesFeature.EnableInterconnect {
		lrsrPredicate := func(item *nbdb.LogicalRouterStaticRoute) bool {
			...
			v4Eps := svcKeyToRemoteV4Endpoints[svcKey]
			v6Eps := svcKeyToRemoteV6Endpoints[svcKey]
		...
		if err != nil {
			errorList = append(errorList,
				fmt.Errorf("error deleting stale logical router static routes from router %s: %v", ovntypes.OVNClusterRouter, err))
		}
	}
...
func (c *Controller) syncEgressService(key string) error {
	...
	//  - create LRPs for local endpoints with transit switch IP as a nextHop
	...
	createOps, err := c.createLogicalRouterPoliciesOps(key, nextHopV4, nextHopV6, v4LocalToAdd, v6LocalToAdd)
	if err != nil {
		return err
	}
	allOps = append(allOps, createOps...)

	if config.OVNKubernetesFeature.EnableInterconnect && svcNodeInLocalZone {
		createOps, err = c.createLogicalRouterStaticRoutesOps(key, node.v4MgmtIP.String(), node.v6MgmtIP.String(), v4RemoteToAdd, v6RemoteToAdd)
		if err != nil {
			return err
		}
		allOps = append(allOps, createOps...)
