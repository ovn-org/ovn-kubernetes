...
func (c *Controller) nodeStateFor(name string) (*nodeState, error) {
	...
	if config.OVNKubernetesFeature.EnableInterconnect {
		...
		if err != nil {
			return nil, fmt.Errorf("unable to fetch transit switch IP for node %s: %w", node, err)
		}
...
// nodeZoneState returns whether the node zone is known and if it is local to the zone controller
// should only be called with controller locked
func (c *Controller) nodeZoneState(nodeName string) (exists bool, isNodeZoneLocal bool) {
	isNodeZoneLocal, exists = c.nodesZoneState[nodeName]
	return
}
