	...
	for _, eps := range endpointSlices {
		...
		for _, ep := range eps.Endpoints {
			...
			if config.OVNKubernetesFeature.EnableInterconnect {
				var zoneKnown bool
				zoneKnown, isEpLocal = c.nodeZoneState(*ep.NodeName)
				if !zoneKnown {
...
// Returns the libovsdb operations to create the logical router static routes for the service,
// given its key, the nexthop (mgmt ip) and endpoints to add.
func (c *Controller) createLogicalRouterStaticRoutesOps(key, v4MgmtIP, v6MgmtIP string, v4Endpoints, v6Endpoints []string) ([]libovsdb.Operation, error) {
	allOps := []libovsdb.Operation{}
	var err error
