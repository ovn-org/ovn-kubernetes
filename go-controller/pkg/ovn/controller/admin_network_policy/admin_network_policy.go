...
func (c *Controller) ensureAdminNetworkPolicy(anp *anpapi.AdminNetworkPolicy) error {
	...
	err = c.updateExistingANP(currentANPState, desiredANPState, atLeastOneRuleUpdated, hasPriorityChanged, false, desiredACLs, desiredPorts)
	if err != nil {
...
func (c *Controller) convertANPRuleToACL(rule *gressRule, anpName string, managedNetworks sets.Set[string],
	...
	for protocol, l4Match := range libovsdbutil.GetL4MatchesFromNetworkPolicyPorts(rule.ports) {
		for netName := range managedNetworks {
			...
			pgName := c.getANPPortGroupName(anpName, netName, false)
...
func (c *Controller) expandRulePeers(rule *gressRule, managedNetworks sets.Set[string]) error {
	...
	for _, peer := range rule.peers {
		...
		for _, node := range nodes {
			...
			for _, peerAddresses := range networkScopedPeerAddresses {
				peerAddresses.Insert(nodeIPs...)
			}
		...
		for _, peerAddresses := range networkScopedPeerAddresses {
			peerAddresses.Insert(peer.networks.UnsortedList()...)
		}
	}
...
func (c *Controller) updateExistingANP(currentANPState, desiredANPState *adminNetworkPolicyState, atLeastOneRuleUpdated,
	hasPriorityChanged, isBanp bool, desiredACLs map[string][]*nbdb.ACL, desiredPorts map[string][]*nbdb.LogicalSwitchPort) error {
	...
	if err := c.updatePortGroupsForNetworkChanges(networksToAdd, networksToDelete, desiredANPState.name, isBanp, desiredPorts); err != nil {
	...
	if fullPeerRecompute || atLeastOneRuleUpdated || hasPriorityChanged || hasACLLoggingParamsChanged || len(networksToAdd) > 0 {
		...
		for networkName, acls := range desiredACLs {
	...
	subjectOps, err := c.constructOpsForSubjectChanges(currentANPState, desiredANPState)
	if err != nil {
...
func (c *Controller) updatePortGroupsForNetworkChanges(networksToAdd, networksToDelete sets.Set[string],
	anpName string, isBanp bool, desiredPorts map[string][]*nbdb.LogicalSwitchPort) error {
	...
	for networkName := range networksToAdd {
		...
		pg := libovsdbutil.BuildPortGroup(pgDbIDs, desiredPorts[networkName], nil)
	...
	for networkName := range networksToDelete {
		portGroupName := c.getANPPortGroupName(anpName, networkName, false)
...
func (c *Controller) constructOpsForPeerChanges(desiredRules, currentRules []*gressRule,
	...
	for i := range desiredRules {
		...
		for networkName, desiredPeerAddresses := range desiredRule.peerAddresses {
			currentPeerAddresses, ok := currentRule.peerAddresses[networkName]
			if !ok {
				// network add event: so let's set the current value to empty set for this network
...
func (c *Controller) constructOpsForSubjectChanges(currentANPState, desiredANPState *adminNetworkPolicyState) ([]ovsdb.Operation, error) {
	var ops []ovsdb.Operation
	var err error
	...
	for networkName, desiredANPStatePodPorts := range desiredANPState.subject.podPorts {
		currentANPStatePodPorts, ok := currentANPState.subject.podPorts[networkName]
		if !ok {
			// means it is a new network; nothing to-do because updatePortGroupsForNetworkChanges
			// has already handled this case
			continue
		}
		...
		portGroupName := c.getANPPortGroupName(desiredANPState.name, networkName, false)
		if len(portsToAdd) > 0 {
