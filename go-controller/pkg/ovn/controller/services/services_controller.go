...
func (c *Controller) syncServices(key string) error {
	...
	// Apply the load balancers to the cluster, or short-circuit if nothing should change
	existingLBs, ok := c.alreadyApplied.Load(key)
	if ok && reflect.DeepEqual(lbs, existingLBs) {
		...
		c.alreadyApplied.Store(key, lbs)
	}
