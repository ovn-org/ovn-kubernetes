	...
	tests := []struct {
		...
		},
		{
			...
			ovnCmd: []ovntest.ExpectedCmd{
				...
				{
					Cmd:    `ovn-nbctl --timeout=15 --no-heading --format=csv --data=bare --columns=name find logical_switch load_balancer{>=}uuid-1`,
					Output: `wrong-switch`,
				},
				{
					Cmd: `ovn-nbctl --timeout=15 --may-exist ls-lb-add switch-node-a uuid-1 -- --may-exist ls-lb-add switch-node-b uuid-1 -- --if-exists ls-lb-del wrong-switch uuid-1 -- --may-exist lr-lb-add gr-node-b uuid-1 -- --if-exists lr-lb-del gr-node-c uuid-1`,
				},
				{
					Cmd: `ovn-nbctl --timeout=15 --no-heading --format=csv --data=json --columns=_uuid,name,external_ids,vips,protocol find load_balancer`,
				},
		...
		},
		{
			...
			ovnCmd: []ovntest.ExpectedCmd{
				...
				{
					Cmd: `ovn-nbctl --timeout=15 --may-exist ls-lb-add switch-node-a uuid-rs-nodea -- --may-exist lr-lb-add gr-node-a uuid-rs-nodea`,
				},
				...
				{
					Cmd: `ovn-nbctl --timeout=15 --may-exist ls-lb-add switch-node-b uuid-rs-nodeb -- --may-exist lr-lb-add gr-node-b uuid-rs-nodeb`,
				},
