...
func getAllLBs(nbClient libovsdbclient.Client, allTemplates TemplateMap, netInfo util.NetInfo) ([]*LB, error) {
	_, out, err := _getLBsCommon(nbClient, allTemplates, false, true, netInfo)
	return out, err
}

// getServiceLBsForNetwork returns a set of services as well as a slice of load balancers found in OVN.
