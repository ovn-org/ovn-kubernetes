...
func defaultFakeExec(nodeSubnet, nodeName string, sctpSupport bool) (*ovntest.FakeExec, string, string, string) {
	...
	cmd := "ovn-nbctl --timeout=15 -- --if-exists lrp-del " + routerToSwitchPrefix + nodeName +
		" -- lrp-add ovn_cluster_router " + routerToSwitchPrefix + nodeName + " " + lrpMAC + " " + gwCIDR +
		" -- --may-exist ls-add " + nodeName +
		" -- set logical_switch " + nodeName + " other-config:subnet=" + nodeSubnet + " other-config:exclude_ips=" + nodeMgmtPortIP.String() + ".." + hybridOverlayIP.String() +
		" -- set logical_switch " + nodeName + " other-config:mcast_snoop=\"true\"" +
		" -- set logical_switch " + nodeName + " other-config:mcast_querier=\"true\" other-config:mcast_eth_src=\"" + lrpMAC + "\" other-config:mcast_ip4_src=\"" + gwIP + "\"" +
		" -- --may-exist lsp-add " + nodeName + " " + switchToRouterPrefix + nodeName + " -- set logical_switch_port " + switchToRouterPrefix + nodeName + " type=router options:router-port=" + routerToSwitchPrefix + nodeName + " addresses=\"" + lrpMAC + "\"" +
		" -- set logical_switch " + nodeName + " load_balancer=" + tcpLBUUID +
		" -- add logical_switch " + nodeName + " load_balancer " + udpLBUUID
	if sctpSupport {
		cmd = cmd + " -- add logical_switch " + nodeName + " load_balancer " + sctpLBUUID
	}
	fexec.AddFakeCmdsNoOutputNoError([]string{cmd})

	fexec.AddFakeCmdsNoOutputNoError([]string{
		"ovn-nbctl --timeout=15 -- --may-exist acl-add " + nodeName + " to-lport 1001 ip4.src==" + nodeMgmtPortIP.String() + " allow-related" +
			" -- --may-exist lsp-add " + nodeName + " " + util.K8sPrefix + nodeName +
			" -- lsp-set-addresses " + util.K8sPrefix + nodeName + " " + mgmtMAC + " " + nodeMgmtPortIP.String(),
	})
	fexec.AddFakeCmd(&ovntest.ExpectedCmd{
...
`, node1Name, node1Subnet, masterName, masterSubnet),
			...
			fexec.AddFakeCmdsNoOutputNoError([]string{
				"ovn-nbctl --timeout=15 -- --if-exists lrp-del " + routerToSwitchPrefix + masterName +
					" -- lrp-add ovn_cluster_router " + routerToSwitchPrefix + masterName + " " + lrpMAC + " " + masterGWCIDR +
					" -- --may-exist ls-add " + masterName + " -- set logical_switch " + masterName + " other-config:subnet=" + masterSubnet + " other-config:exclude_ips=" + masterMgmtPortIP +
					" -- --may-exist lsp-add " + masterName + " " + switchToRouterPrefix + masterName + " -- set logical_switch_port " + switchToRouterPrefix + masterName + " type=router options:router-port=" + routerToSwitchPrefix + masterName + " addresses=\"" + lrpMAC + "\"" +
					" -- set logical_switch " + masterName + " load_balancer=" + tcpLBUUID +
					" -- add logical_switch " + masterName + " load_balancer " + udpLBUUID +
					" -- add logical_switch " + masterName + " load_balancer " + sctpLBUUID,
			})
			fexec.AddFakeCmdsNoOutputNoError([]string{
				"ovn-nbctl --timeout=15 -- --may-exist acl-add " + masterName + " to-lport 1001 ip4.src==" + masterMgmtPortIP + " allow-related" +
					" -- --may-exist lsp-add " + masterName + " " + util.K8sPrefix + masterName +
					" -- lsp-set-addresses " + util.K8sPrefix + masterName + " " + masterMgmtPortMAC + " " + masterMgmtPortIP,
			})
			fexec.AddFakeCmd(&ovntest.ExpectedCmd{
...
var _ = Describe("Gateway Init Operations", func() {
	...
	It("sets up a localnet gateway", func() {
		app.Action = func(ctx *cli.Context) error {
			...
			fexec.AddFakeCmdsNoOutputNoError([]string{
				"ovn-nbctl --timeout=15 -- --if-exists lrp-del " + routerToSwitchPrefix + nodeName +
					" -- lrp-add ovn_cluster_router " + routerToSwitchPrefix + nodeName + " " + nodeLRPMAC + " " + masterGWCIDR +
					" -- --may-exist ls-add " + nodeName + " -- set logical_switch " + nodeName + " other-config:subnet=" + nodeSubnet + " other-config:exclude_ips=" + masterMgmtPortIP +
					" -- --may-exist lsp-add " + nodeName + " " + switchToRouterPrefix + nodeName + " -- set logical_switch_port " + switchToRouterPrefix + nodeName + " type=router options:router-port=" + routerToSwitchPrefix + nodeName + " addresses=\"" + nodeLRPMAC + "\"" +
					" -- set logical_switch " + nodeName + " load_balancer=" + tcpLBUUID +
					" -- add logical_switch " + nodeName + " load_balancer " + udpLBUUID +
					" -- add logical_switch " + nodeName + " load_balancer " + sctpLBUUID,
			})
			fexec.AddFakeCmdsNoOutputNoError([]string{
				"ovn-nbctl --timeout=15 -- --may-exist acl-add " + nodeName + " to-lport 1001 ip4.src==" + masterMgmtPortIP + " allow-related" +
					" -- --may-exist lsp-add " + nodeName + " " + util.K8sPrefix + nodeName +
					" -- lsp-set-addresses " + util.K8sPrefix + nodeName + " " + brLocalnetMAC + " " + masterMgmtPortIP,
			})
			fexec.AddFakeCmd(&ovntest.ExpectedCmd{
				Cmd:    "ovn-nbctl --timeout=15 lsp-list " + nodeName,
				Output: "29df5ce5-2802-4ee5-891f-4fb27ca776e9 (" + util.K8sPrefix + nodeName + ")",
			...
			fexec.AddFakeCmdsNoOutputNoError([]string{
				...
				"ovn-nbctl --timeout=15 -- --may-exist lr-add " + gwRouter +
					" -- set logical_router " + gwRouter + " options:chassis=" + systemID + " external_ids:physical_ip=169.254.33.2 external_ids:physical_ips=169.254.33.2" +
					" -- --may-exist ls-add " + joinSwitch +
					" -- --may-exist lsp-add " + joinSwitch + " " + joinSwitchToGwRouterPrefix + gwRouter +
					" -- set logical_switch_port " + joinSwitchToGwRouterPrefix + gwRouter + " type=router options:router-port=" + gwRouterToJoinSwitchPrefix + gwRouter + " addresses=router" +
					" -- --if-exists lrp-del " + gwRouterToJoinSwitchPrefix + gwRouter +
					" -- lrp-add " + gwRouter + " " + gwRouterToJoinSwitchPrefix + gwRouter + " " + lrpMAC + " " + lrpIP + "/29" +
					" -- --may-exist lsp-add " + joinSwitch + " " + joinSwitchToDistRouterPrefix + nodeName +
					" -- set logical_switch_port " + joinSwitchToDistRouterPrefix + nodeName + " type=router options:router-port=" + distRouterToJoinSwitchPrefix + nodeName + " addresses=router" +
					" -- --if-exists lrp-del " + distRouterToJoinSwitchPrefix + nodeName +
					" -- lrp-add " + ovnClusterRouter + " " + distRouterToJoinSwitchPrefix + nodeName + " " + drLrpMAC + " " + drLrpIP + "/29" +
					" -- set logical_router " + gwRouter + " options:lb_force_snat_ip=" + lrpIP +
					" -- --may-exist lr-route-add " + gwRouter + " " + clusterCIDR + " " + drLrpIP,
			})
			...
			fexec.AddFakeCmdsNoOutputNoError([]string{
				"ovn-nbctl --timeout=15 -- --may-exist ls-add " + externalSwitchPrefix + nodeName +
					" -- --may-exist lsp-add " + externalSwitchPrefix + nodeName + " br-local_" + nodeName +
					" -- lsp-set-addresses br-local_" + nodeName + " unknown" +
					" -- lsp-set-type br-local_" + nodeName + " localnet" +
					" -- lsp-set-options br-local_" + nodeName + " network_name=" + util.PhysicalNetworkName +
					" -- --if-exists lrp-del " + gwRouterToExtSwitchPrefix + gwRouter +
					" -- lrp-add " + gwRouter + " " + gwRouterToExtSwitchPrefix + gwRouter + " " + brLocalnetMAC + " 169.254.33.2/24" +
					" -- set logical_router_port " + gwRouterToExtSwitchPrefix + gwRouter + " external-ids:gateway-physical-ip=yes" +
					" -- --may-exist lsp-add " + externalSwitchPrefix + nodeName + " " + extSwitchToGwRouterPrefix + gwRouter +
					" -- set logical_switch_port " + extSwitchToGwRouterPrefix + gwRouter + " type=router options:router-port=" + gwRouterToExtSwitchPrefix + gwRouter + " addresses=\"" + brLocalnetMAC + "\"" +
					" -- --may-exist lr-route-add " + gwRouter + " 0.0.0.0/0 169.254.33.1 " + gwRouterToExtSwitchPrefix + gwRouter +
					" -- --may-exist --policy=src-ip lr-route-add " + ovnClusterRouter + " " + nodeSubnet + " " + lrpIP +
					" -- --may-exist lr-nat-add " + gwRouter + " snat 169.254.33.2 " + clusterCIDR,
			})
			fexec.AddFakeCmd(&ovntest.ExpectedCmd{
	...
	It("sets up a shared gateway", func() {
		app.Action = func(ctx *cli.Context) error {
			...
			fexec.AddFakeCmdsNoOutputNoError([]string{
				"ovn-nbctl --timeout=15 -- --if-exists lrp-del " + routerToSwitchPrefix + nodeName +
					" -- lrp-add ovn_cluster_router " + routerToSwitchPrefix + nodeName + " " + nodeLRPMAC + " " + nodeGWIP +
					" -- --may-exist ls-add " + nodeName + " -- set logical_switch " + nodeName + " other-config:subnet=" + nodeSubnet + " other-config:exclude_ips=" + nodeMgmtPortIP +
					" -- --may-exist lsp-add " + nodeName + " " + switchToRouterPrefix + nodeName + " -- set logical_switch_port " + switchToRouterPrefix + nodeName + " type=router options:router-port=" + routerToSwitchPrefix + nodeName + " addresses=\"" + nodeLRPMAC + "\"" +
					" -- set logical_switch " + nodeName + " load_balancer=" + tcpLBUUID +
					" -- add logical_switch " + nodeName + " load_balancer " + udpLBUUID +
					" -- add logical_switch " + nodeName + " load_balancer " + sctpLBUUID,
			...
			fexec.AddFakeCmdsNoOutputNoError([]string{
				"ovn-nbctl --timeout=15 -- --may-exist acl-add " + nodeName + " to-lport 1001 ip4.src==" + nodeMgmtPortIP + " allow-related" +
					" -- --may-exist lsp-add " + nodeName + " " + util.K8sPrefix + nodeName +
					" -- lsp-set-addresses " + util.K8sPrefix + nodeName + " " + nodeMgmtPortMAC + " " + nodeMgmtPortIP,
			})
			fexec.AddFakeCmd(&ovntest.ExpectedCmd{
				Cmd:    "ovn-nbctl --timeout=15 lsp-list " + nodeName,
				Output: "29df5ce5-2802-4ee5-891f-4fb27ca776e9 (" + util.K8sPrefix + nodeName + ")",
			...
			fexec.AddFakeCmdsNoOutputNoError([]string{
				"ovn-nbctl --timeout=15 -- --if-exists remove logical_switch " + nodeName + " other-config exclude_ips",
				"ovn-nbctl --timeout=15 -- --may-exist lr-add " + gwRouter +
					" -- set logical_router " + gwRouter + " options:chassis=" + systemID + " external_ids:physical_ip=" + gatewayRouterIP + " external_ids:physical_ips=" + gatewayRouterIP +
					" -- --may-exist ls-add " + joinSwitch +
					" -- --may-exist lsp-add " + joinSwitch + " " + joinSwitchToGwRouterPrefix + gwRouter +
					" -- set logical_switch_port " + joinSwitchToGwRouterPrefix + gwRouter + " type=router options:router-port=" + gwRouterToJoinSwitchPrefix + gwRouter + " addresses=router" +
					" -- --if-exists lrp-del " + gwRouterToJoinSwitchPrefix + gwRouter +
			...
			fexec.AddFakeCmdsNoOutputNoError([]string{
				"ovn-nbctl --timeout=15 -- --may-exist ls-add " + externalSwitchPrefix + nodeName +
					" -- --may-exist lsp-add " + externalSwitchPrefix + nodeName + " br-eth0_" + nodeName +
					" -- lsp-set-addresses br-eth0_" + nodeName + " unknown" +
					" -- lsp-set-type br-eth0_" + nodeName + " localnet" +
					" -- lsp-set-options br-eth0_" + nodeName + " network_name=" + util.PhysicalNetworkName +
					" -- set logical_switch_port br-eth0_" + nodeName + " tag_request=" + "1024" +
					" -- --if-exists lrp-del " + gwRouterToExtSwitchPrefix + gwRouter +
					" -- lrp-add " + gwRouter + " " + gwRouterToExtSwitchPrefix + gwRouter + " " + physicalBridgeMAC + " " + gatewayRouterIPMask +
					" -- set logical_router_port " + gwRouterToExtSwitchPrefix + gwRouter + " external-ids:gateway-physical-ip=yes" +
					" -- --may-exist lsp-add " + externalSwitchPrefix + nodeName + " " + extSwitchToGwRouterPrefix + gwRouter +
					" -- set logical_switch_port " + extSwitchToGwRouterPrefix + gwRouter + " type=router options:router-port=" + gwRouterToExtSwitchPrefix + gwRouter + " addresses=\"" + physicalBridgeMAC + "\"" +
					" -- --may-exist lr-route-add " + gwRouter + " 0.0.0.0/0 " + gatewayRouterNextHop + " " + gwRouterToExtSwitchPrefix + gwRouter +
					" -- --may-exist --policy=src-ip lr-route-add " + ovnClusterRouter + " " + nodeSubnet + " " + lrpIP +
					" -- --may-exist lr-nat-add " + gwRouter + " snat " + gatewayRouterIP + " " + clusterCIDR,
			})
