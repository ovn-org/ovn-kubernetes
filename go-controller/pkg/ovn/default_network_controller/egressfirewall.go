...
func (oc *DefaultNetworkController) addEgressFirewallRules(ef *egressFirewall, hashedAddressSetNameIPv4, hashedAddressSetNameIPv6 string, efStartPriority int, aclLogging *bnc.ACLLoggingLevels) error {
	for _, rule := range ef.egressRules {
		var action string
...
func (oc *DefaultNetworkController) createEgressFirewallRules(priority int, match, action, externalID string, aclLogging *bnc.ACLLoggingLevels) error {
...
func (oc *DefaultNetworkController) updateACLLoggingForEgressFirewall(egressFirewallNamespace string, nsInfo *bnc.NamespaceInfo) (bool, error) {
