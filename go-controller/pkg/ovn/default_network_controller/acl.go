...
func BuildACL(aclName string, priority int, match, action string,
	logLevels *bnc.ACLLoggingLevels, aclT aclType, externalIDs map[string]string) *nbdb.ACL {
...
func (oc *DefaultNetworkController) GetNamespaceACLLogging(ns string) *bnc.ACLLoggingLevels {
	nsInfo, nsUnlock := oc.GetNamespaceLocked(ns, true)
	if nsInfo == nil {
		return &bnc.ACLLoggingLevels{
			Allow: "",
			Deny:  "",
		}
	}
...
func getLogSeverity(action string, aclLogging *bnc.ACLLoggingLevels) (log bool, severity string) {
	severity = ""
	if aclLogging != nil {
...
func UpdateACLLoggingWithPredicate(nbClient libovsdbclient.Client, p func(*nbdb.ACL) bool, aclLogging *bnc.ACLLoggingLevels) error {
...
func UpdateACLLogging(nbClient libovsdbclient.Client, ACLs []*nbdb.ACL, aclLogging *bnc.ACLLoggingLevels) error {
	if len(ACLs) == 0 {
		return nil
	}
	for i := range ACLs {
