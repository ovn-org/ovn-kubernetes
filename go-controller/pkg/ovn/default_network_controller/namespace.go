...
func (oc *DefaultNetworkController) getRoutingExternalGWs(nsInfo *bnc.NamespaceInfo) *bnc.GatewayInfo {
	res := bnc.GatewayInfo{}
...
func validateRoutingPodGWs(podGWs map[string]bnc.GatewayInfo) error {
...
func (oc *DefaultNetworkController) getRoutingPodGWs(nsInfo *bnc.NamespaceInfo) map[string]bnc.GatewayInfo {
	// return a copy of the object so it can be handled without the
	// namespace locked
	res := make(map[string]bnc.GatewayInfo)
	for k, v := range nsInfo.RoutingExternalPodGWs {
		item := bnc.GatewayInfo{
...
func (oc *DefaultNetworkController) addPodToNamespace(ns string, ips []*net.IPNet) (*bnc.GatewayInfo, map[string]bnc.GatewayInfo, []ovsdb.Operation, error) {
	...
	if ops, err = nsInfo.AddressSet.AddIPsReturnOps(ovnutil.CreateIPAddressSlice(ips)); err != nil {
...
func (oc *DefaultNetworkController) multicastUpdateNamespace(ns *kapi.Namespace, nsInfo *bnc.NamespaceInfo) error {
...
// previously allowed.
func (oc *DefaultNetworkController) multicastDeleteNamespace(ns *kapi.Namespace, nsInfo *bnc.NamespaceInfo) error {
...
func (oc *DefaultNetworkController) configureNamespace(nsInfo *bnc.NamespaceInfo, ns *kapi.Namespace) error {
	var errors []error
	if annotation, ok := ns.Annotations[util.RoutingExternalGWsAnnotation]; ok {
		...
		if err != nil {
			...
			err = oc.addExternalGWsForNamespace(bnc.GatewayInfo{GWs: exGateways, BFDEnabled: bfdEnabled}, nsInfo, ns.Name)
			if err != nil {
...
func (oc *DefaultNetworkController) updateNamespace(old, newer *kapi.Namespace) error {
	...
	if gwAnnotation != oldGWAnnotation || newBFDEnabled != oldBFDEnabled {
		...
		if oldGWAnnotation == "" {
			...
			nsInfo.RoutingExternalGWs = bnc.GatewayInfo{}
		}
		exGateways, err := util.ParseRoutingExternalGWAnnotation(gwAnnotation)
		if err != nil {
			errors = append(errors, err)
		} else {
			err = oc.addExternalGWsForNamespace(bnc.GatewayInfo{GWs: exGateways, BFDEnabled: newBFDEnabled}, nsInfo, old.Name)
			if err != nil {
				errors = append(errors, err)
			}
		}
...
func (oc *DefaultNetworkController) ensureNamespaceLocked(ns string, readOnly bool, namespace *kapi.Namespace) (*bnc.NamespaceInfo, func(), error) {
	oc.NamespacesMutex.Lock()
	nsInfo := oc.Namespaces[ns]
	nsInfoExisted := false
	if nsInfo == nil {
		nsInfo = &bnc.NamespaceInfo{
			RelatedNetworkPolicies: map[string]bool{},
			MulticastEnabled:       false,
			RoutingExternalPodGWs:  make(map[string]bnc.GatewayInfo),
			RoutingExternalGWs:     bnc.GatewayInfo{GWs: sets.NewString(), BFDEnabled: false},
		}
