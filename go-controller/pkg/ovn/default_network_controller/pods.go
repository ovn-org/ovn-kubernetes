...
func (oc *DefaultNetworkController) deleteLogicalPort(pod *kapi.Pod, portInfo *ovnutil.LPInfo) (err error) {
...
func (oc *DefaultNetworkController) addLogicalPort(pod *kapi.Pod) (err error) {
	...
	gateways := make([]*bnc.GatewayInfo, 0, len(routingExternalGWs.GWs)+len(routingPodGWs))
