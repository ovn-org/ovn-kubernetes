...
import (
	...
	"github.com/ovn-org/ovn-kubernetes/go-controller/pkg/ovn/default_network_controller/controller/unidling"
...
func NewDefaultNetworkController(cnci *bnc.CommonNetworkControllerInfo) *DefaultNetworkController {
	stopChan := make(chan struct{})
	wg := &sync.WaitGroup{}
	return newDefaultNetworkControllerCommon(cnci, stopChan, wg, nil)
}

func newDefaultNetworkControllerCommon(cnci *bnc.CommonNetworkControllerInfo,
	...
	oc := &DefaultNetworkController{
		BaseNetworkController: bnc.BaseNetworkController{
			...
			LogicalPortCache:            ovnutil.NewPortCache(defaultStopChan),
...
func (h *defaultNetworkControllerEventHandler) UpdateResource(oldObj, newObj interface{}, inRetryCache bool) error {
	...
	case factory.NodeType:
		...
		mgmtSync := failed || macAddressChanged(oldNode, newNode) || ovnutil.NodeSubnetChanged(oldNode, newNode)
		_, failed = h.oc.gatewaysFailed.Load(newNode.Name)
		gwSync := (failed || gatewayChanged(oldNode, newNode) ||
			ovnutil.NodeSubnetChanged(oldNode, newNode) || hostAddressesChanged(oldNode, newNode) ||
			nodeGatewayMTUSupportChanged(oldNode, newNode))
		_, hoSync := h.oc.hybridOverlayFailed.Load(newNode.Name)

		return h.oc.addUpdateNodeEvent(newNode,
			&bnc.NodeSyncs{
				SyncNode:              nodeSync,
				SyncClusterRouterPort: clusterRtrSync,
				SyncMgmtPort:          mgmtSync,
...
func (h *defaultNetworkControllerEventHandler) DeleteResource(obj, cachedObj interface{}) error {
	...
	case factory.PodType:
		...
		if cachedObj != nil {
			portInfo = cachedObj.(*ovnutil.LPInfo)
		}
