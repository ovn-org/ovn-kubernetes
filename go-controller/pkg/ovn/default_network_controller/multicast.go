...
func getMulticastACLIgrMatch(nsInfo *bnc.NamespaceInfo) string {
...
func (oc *DefaultNetworkController) createMulticastAllowPolicy(ns string, nsInfo *bnc.NamespaceInfo) error {
	portGroupName := util.HashedPortGroup(ns)
...
// lock.
func podAddAllowMulticastPolicy(nbClient libovsdbclient.Client, ns string, portInfo *ovnutil.LPInfo) error {
	return libovsdbops.AddPortsToPortGroup(nbClient, util.HashedPortGroup(ns), portInfo.UUID)
}
