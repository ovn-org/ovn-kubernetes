...
func generateGatewayInitExpectedNB(testData []libovsdb.TestData, expectedOVNClusterRouter *nbdb.LogicalRouter,
	...
	meters := map[string]string{
		types.OVNARPRateLimiter:              ovnutil.GetMeterNameForProtocol(types.OVNARPRateLimiter),
		types.OVNARPResolveRateLimiter:       ovnutil.GetMeterNameForProtocol(types.OVNARPResolveRateLimiter),
		types.OVNBFDRateLimiter:              ovnutil.GetMeterNameForProtocol(types.OVNBFDRateLimiter),
		types.OVNControllerEventsRateLimiter: ovnutil.GetMeterNameForProtocol(types.OVNControllerEventsRateLimiter),
		types.OVNICMPV4ErrorsRateLimiter:     ovnutil.GetMeterNameForProtocol(types.OVNICMPV4ErrorsRateLimiter),
		types.OVNICMPV6ErrorsRateLimiter:     ovnutil.GetMeterNameForProtocol(types.OVNICMPV6ErrorsRateLimiter),
		types.OVNRejectRateLimiter:           ovnutil.GetMeterNameForProtocol(types.OVNRejectRateLimiter),
		types.OVNTCPRSTRateLimiter:           ovnutil.GetMeterNameForProtocol(types.OVNTCPRSTRateLimiter),
	}
	fairness := true
...
var _ = ginkgo.Describe("Gateway Init Operations", func() {
	...
	ginkgo.Context("Gateway Creation Operations Shared Gateway Mode", func() {
		...
		ginkgo.It("creates an IPv4 gateway in OVN", func() {
			...
			var err error
			fakeOvn.controller.defaultCOPPUUID, err = ovnutil.EnsureDefaultCOPP(fakeOvn.nbClient)
