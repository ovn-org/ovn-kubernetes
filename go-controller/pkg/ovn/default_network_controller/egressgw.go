...
func (oc *DefaultNetworkController) addPodExternalGW(pod *kapi.Pod) error {
	...
	for _, namespace := range strings.Split(podRoutingNamespaceAnno, ",") {
		err := oc.addPodExternalGWForNamespace(namespace, pod, bnc.GatewayInfo{GWs: foundGws, BFDEnabled: enableBFD})
		if err != nil {
			return err
		}
	}
...
func (oc *DefaultNetworkController) addPodExternalGWForNamespace(namespace string, pod *kapi.Pod, egress bnc.GatewayInfo) error {
...
func (oc *DefaultNetworkController) addExternalGWsForNamespace(egress bnc.GatewayInfo, nsInfo *bnc.NamespaceInfo, namespace string) error {
	if egress.GWs == nil {
...
func (oc *DefaultNetworkController) addGWRoutesForNamespace(namespace string, egress bnc.GatewayInfo) error {
	...
	for _, pod := range existingPods {
		...
		if err := oc.addGWRoutesForPod([]*bnc.GatewayInfo{&egress}, podIPs, podNsName, pod.Spec.NodeName); err != nil {
			return err
		}
	}
	return nil
...
func (oc *DefaultNetworkController) addGWRoutesForPod(gateways []*bnc.GatewayInfo, podIfAddrs []*net.IPNet, podNsName ktypes.NamespacedName, node string) error {
