...
func (oc *DefaultNetworkController) syncNetworkPolicies(networkPolicies []interface{}) error {
	...
	err := oc.AddressSetFactory.ProcessEachAddressSet(func(_, addrSetName string) error {
		...
		if !expectedPolicies[namespaceName][policyName] {
			...
			hashedLocalPortGroup := util.HashedPortGroup(portGroupName)
...
func defaultDenyPortGroupName(namespace, gressSuffix string) string {
	return util.HashedPortGroup(namespace) + "_" + gressSuffix
}

func buildDenyACLs(namespace, pg string, aclLogging *bnc.ACLLoggingLevels, aclT aclType) (denyACL, allowACL *nbdb.ACL) {
...
func (oc *DefaultNetworkController) addPolicyToDefaultPortGroups(np *networkPolicy, aclLogging *bnc.ACLLoggingLevels) error {
...
func (oc *DefaultNetworkController) createDefaultDenyPGAndACLs(namespace, policy string, aclLogging *bnc.ACLLoggingLevels) error {
	ingressPGName := defaultDenyPortGroupName(namespace, ingressDefaultDenySuffix)
...
func (oc *DefaultNetworkController) updateACLLoggingForPolicy(np *networkPolicy, aclLogging *bnc.ACLLoggingLevels) error {
	np.RLock()
	defer np.RUnlock()
	if np.deleted {
		return nil
...
func (oc *DefaultNetworkController) updateACLLoggingForDefaultACLs(ns string, nsInfo *bnc.NamespaceInfo) error {
...
func (oc *DefaultNetworkController) handleNetPolNamespaceUpdate(namespace string, nsInfo *bnc.NamespaceInfo) error {
...
func getNetworkPolicyPGName(namespace, name string) (pgName, readablePGName string) {
	...
	return util.HashedPortGroup(readableGroupName), readableGroupName
}
...
// that information.
func (oc *DefaultNetworkController) createNetworkPolicy(policy *knet.NetworkPolicy, aclLogging *bnc.ACLLoggingLevels) (*networkPolicy, error) {
...
func (oc *DefaultNetworkController) buildNetworkPolicyACLs(np *networkPolicy, aclLogging *bnc.ACLLoggingLevels) []*nbdb.ACL {
	acls := []*nbdb.ACL{}
...
func (oc *DefaultNetworkController) peerNamespaceUpdate(np *networkPolicy, gp *gressPolicy) error {
	...
	var aclLogging *bnc.ACLLoggingLevels
	if nsInfo == nil {
		aclLogging = &bnc.ACLLoggingLevels{
			Allow: "",
			Deny:  "",
		}
	} else {
