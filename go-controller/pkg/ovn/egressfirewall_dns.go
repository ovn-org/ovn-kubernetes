...
func (e *EgressDNS) Delete(namespace string) (bool, error) {
	e.lock.Lock()
	...
	for dnsName, dnsEntry := range e.dnsEntries {
		...
		if len(dnsEntry.namespaces) == 0 {
			...
			if err != nil {
				return false, fmt.Errorf("error deleting EgressFirewall AddressSet for dnsName: %s %v", dnsName, err)
			}
