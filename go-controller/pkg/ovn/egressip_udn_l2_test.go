...
"default":{"mode":"local","mac-address":"7e:57:f8:f0:3c:49", "ip-address":"192.168.126.12/24", "next-hop": "192.168.126.1", "next-hops": ["192.168.126.1"]}}`, networkName1, v4Net1, masqIP, masqIP),
				}
...
"default":{"mode":"local","mac-address":"7e:57:f8:f0:3c:49", "ip-address":"192.168.126.12/24", "next-hop":"192.168.126.1", "next-hops": ["192.168.126.1"]}`, networkName1, v4Net1, masqIP, masqIP),
				...
				initialDB := []libovsdbtest.TestData{
					...
					&nbdb.LogicalRouter{
						...
						Policies:    []string{getGWPktMarkLRPUUID(eipNamespace2, podName4, IPFamilyValueV4, networkName1)}, // stale policy
					},
					&nbdb.LogicalSwitchPort{
				...
				expectedDatabaseStateTwoEgressNodes := []libovsdbtest.TestData{
					...
					// UDN
					getReRoutePolicyForController(egressIPName, eipNamespace2, podName2, v4Pod1IPNode1Net1, eIP1Mark, IPFamilyValueV4, []string{masqIP, node2Network1RtoSIP}, netInfo.GetNetworkName(), DefaultNetworkControllerName),
...
"default":{"mode":"local","mac-address":"7e:57:f8:f0:3c:49", "ip-address":"192.168.126.12/24", "next-hop": "192.168.126.1", "next-hops": ["192.168.126.1"]}}`, networkName1, v4Net1, masqIP, masqIP),
				...
				err = fakeOvn.eIPController.ensureRouterPoliciesForNetwork(secConInfo.bnc.NetInfo)
				gomega.Expect(err).NotTo(gomega.HaveOccurred())
				err = fakeOvn.eIPController.ensureSwitchPoliciesForNode(secConInfo.bnc.NetInfo, node1Name)
				gomega.Expect(err).NotTo(gomega.HaveOccurred())
				err = fakeOvn.controller.WatchEgressIPNamespaces()
				gomega.Expect(err).NotTo(gomega.HaveOccurred())
				err = fakeOvn.controller.WatchEgressIPPods()
