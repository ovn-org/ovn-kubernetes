...
func (bnc *BaseNetworkController) EnsurePodSelectorPortGroup(podSelector *metav1.LabelSelector, namespace string,
	acls []*nbdb.ACL, backRef string) (portGroupKey string, err error) {
	...
	err = bnc.sharedPodSelectorPortGroups.DoWithLock(portGroupKey, func(key string) error {
		...
		if !found {
			...
			err = psPortGroup.init(bnc, acls, backRef)
		...
		if psPortGroup.needsCleanup {
			...
			err = psPortGroup.addACLs(bnc, acls, backRef)
			if err != nil {
...
func (pspg *PodSelectorPortGroup) init(bnc *BaseNetworkController, acls []*nbdb.ACL, ref string) error {
	...
	if pspg.handlerResources == nil {
		...
		recordOps, txOkCallBack, _, err := bnc.AddConfigDurationRecord("networkpolicy", pspg.namespace, ref)
		if err != nil {
...
func (pspg *PodSelectorPortGroup) addACLs(bnc *BaseNetworkController, acls []*nbdb.ACL, ref string) error {
	ops, err := libovsdbops.CreateOrUpdateACLsOps(bnc.nbClient, nil, bnc.GetSamplingConfig(), acls...)
	if err != nil {
		return fmt.Errorf("failed to create ACL ops: %v", err)
	}
