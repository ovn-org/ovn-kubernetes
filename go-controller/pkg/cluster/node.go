...
func (cluster *OvnClusterController) StartClusterNode(name string) error {
	...
	if cniConfig == "" {
		...
		logrus.Warnf("cni_network_config is not defined in ConfigMap %s/ovn-config, using default config of %s instead",
			config.Kubernetes.OVNConfigNamespace, cniConfig)
	}
