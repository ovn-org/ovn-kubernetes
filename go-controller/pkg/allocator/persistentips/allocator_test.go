...
var _ = Describe("Persistent IP allocator operations", func() {
	...
	Context("an existing, but empty IPAMClaim", func() {
		BeforeEach(func() {
			...
			persistentIPAllocator = NewPersistentIPsAllocator(ovnkapiclient, ipAllocator.ForSubnet(subnetName), networkName)
		})
	...
	Context("an IPAllocator having already allocated some addresses", func() {
		...
		BeforeEach(func() {
			...
			persistentIPAllocator = NewPersistentIPsAllocator(ovnkapiclient, ipAllocator, networkName)
		})
