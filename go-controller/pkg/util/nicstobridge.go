...
func RestoreOVSRegularConfig(bridgeName, bypassIface, bypassRep string) error {
	_, stderr, err := RunOVSVsctl("--if-exists", "remove", "bridge", bridgeName, "external_ids", "ovn-bypass-cfg")
	if err != nil {
		return fmt.Errorf("failed to remove external_ids:ovn-bypass-cfg from bridge %s, error %v, stderr %s",
			bridgeName, err, stderr)
	}
	if _, stderr, err = RunOVSVsctl("--if-exists", "del-port", bridgeName, bypassRep); err != nil {
		return fmt.Errorf("failed to delete port %s from bridge %s, error %v, stderr %s", bypassRep, bridgeName, err, stderr)
	}
	...
	if _, stderr, err = ReplaceOFFlows(bridgeName, flows); err != nil {
		return fmt.Errorf("failed to replace flows on bridge %s, error %v, stderr %s", bridgeName, err, stderr)
	}
	...
	return saveIPConfig(bypassLink, bridgeLink, true)
}
