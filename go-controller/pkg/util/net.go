...
func GetLRPAddrs(nbClient client.Client, portName string) ([]*net.IPNet, error) {
	...
	nbClient.WhereCache(func(lrp *nbdb.LogicalRouterPort) bool { return lrp.Name == portName }).List(&result)
	if len(result) != 1 {
		return nil, fmt.Errorf("did not find precisely one router port for port: %s, router ports: %+v", portName, result)
	}
	gwLRPIPs := []*net.IPNet{}
	for _, network := range result[0].Networks {
