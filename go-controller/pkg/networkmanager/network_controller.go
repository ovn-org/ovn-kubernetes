...
func (c *networkController) setAdvertisements(network util.MutableNetInfo) error {
	...
	podAvertisements := map[string][]string{}
	...
	for raName := range raNames {
		...
		for _, node := range nodes {
			...
			if ra.Spec.Advertisements.PodNetwork {
				podAvertisements[node.Name] = append(podAvertisements[node.Name], vrf)
			}
	...
	network.SetPodNetworkAdvertisedVRFs(podAvertisements)
