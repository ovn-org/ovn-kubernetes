...
func (c *nadController) syncNAD(key string, nad *nettypes.NetworkAttachmentDefinition) error {
	...
	// the network
	if ensureNetwork.GetNetworkID() == 0 && nadNetworkName != types.DefaultNetworkName {
...
// controller running in cluster manager also releases here the network ID when
// deleted.
func (c *nadController) handleNetworkID(old util.NetInfo, new util.MutableNetInfo, nad *nettypes.NetworkAttachmentDefinition) error {
	...
	// release old ID
	if old != nil && new == nil && old.GetNetworkName() != types.DefaultNetworkName {
