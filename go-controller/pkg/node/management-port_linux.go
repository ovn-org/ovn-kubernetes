...
func setupManagementPortIPFamilyConfig(routeManager *routemanager.Controller, mpcfg *managementPortConfig, cfg *managementPortIPFamilyConfig) ([]string, error) {
	...
	forwardingITPSNAT := nodeipt.Rule{
		Protocol: protocol,
		Table:    "nat",
		Chain:    iptableMgmPortChain,
		Args: []string{
			...
			"-j", "SNAT",
			"--to-source", cfg.ifAddr.IP.String(),
			"-m", "comment", "--comment", "OVN SNAT to Management Port (FORWARDING)"},
	}
	...
	case true:
		...
		// local traffic so that it doesnt use the host masquerade address
		deleteRules = []nodeipt.Rule{forwardingITPSNAT}
		appendRules = []nodeipt.Rule{localITPSNAT}
	case false:
		appendRules = []nodeipt.Rule{localITPSNAT, forwardingITPSNAT}
	}
