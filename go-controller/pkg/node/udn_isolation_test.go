...
`
	...
	Context("updates open ports", func() {
		...
		It("on restart", func() {
			start(
				newPodWithIPs(nadNamespace, "pod1", []string{"1.1.1.1", "2014:100:200::1"}, util.OpenPort{Protocol: "tcp", Port: intRef(80)}),
				newPodWithIPs(nadNamespace, "pod2", []string{"1.1.1.2"}, util.OpenPort{Protocol: "icmp"}),
				newPodWithIPs(defaultNamespace, "pod3", []string{"1.1.1.3"}),
				primaryNetNAD(nadNamespace, "nad"))
		...
		It("on pod add", func() {
			...
			Expect(err).NotTo(HaveOccurred())
			_, err = fakeClient.KubeClient.CoreV1().Pods(nadNamespace).Create(context.TODO(),
				newPodWithIPs(nadNamespace, "pod2", []string{"1.1.1.2", "2014:100:200::2"}, util.OpenPort{Protocol: "icmp"}), metav1.CreateOptions{})
			Expect(err).NotTo(HaveOccurred())
			Eventually(func() error {
			...
			_, err = fakeClient.KubeClient.CoreV1().Pods(defaultNamespace).Create(context.TODO(),
				newPodWithIPs(defaultNamespace, "pod3", []string{"1.1.1.3", "2014:100:200::3"}, util.OpenPort{Protocol: "icmp"}), metav1.CreateOptions{})
			Expect(err).NotTo(HaveOccurred())
			Consistently(func() error {
				return nodenft.MatchNFTRules(getExpectedDumpWithOpenPorts([]string{"1.1.1.1", "1.1.1.2"}, []string{"2014:100:200::1", "2014:100:200::2"}, map[string][]*util.OpenPort{
					"1.1.1.1":         {{Protocol: "tcp", Port: intRef(80)}},
		...
		It("on pod delete", func() {
			start(
				newPodWithIPs(nadNamespace, "pod1", []string{"1.1.1.1", "2014:100:200::1"}, util.OpenPort{Protocol: "tcp", Port: intRef(80)}),
				newPodWithIPs(nadNamespace, "pod2", []string{"1.1.1.2", "2014:100:200::2"}, util.OpenPort{Protocol: "icmp"}),
				newPodWithIPs(defaultNamespace, "pod3", []string{"1.1.1.2"}),
				primaryNetNAD(nadNamespace, "nad"))
...
func newPodWithIPs(namespace, name string, ips []string, openPorts ...util.OpenPort) *v1.Pod {
