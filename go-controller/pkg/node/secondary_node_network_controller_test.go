...
var _ = Describe("SecondaryNodeNetworkController", func() {
	...
	It("ensure UDNGateway is not invoked when feature gate is OFF", func() {
		...
		Expect(err).NotTo(HaveOccurred())
		controller, err := NewSecondaryNodeNetworkController(&cnnci, NetInfo, nil, nil)
		Expect(err).NotTo(HaveOccurred())
	...
	It("ensure UDNGateway is invoked for Primary UDNs when feature gate is ON but network is not Primary", func() {
		config.OVNKubernetesFeature.EnableNetworkSegmentation = true
		config.OVNKubernetesFeature.EnableMultiNetwork = true
		factoryMock := factoryMocks.NodeWatchFactory{}
		nodeList := []*corev1.Node{
