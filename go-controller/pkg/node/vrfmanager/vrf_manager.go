...
func (vrfm *Controller) Run(stopCh <-chan struct{}, doneWg *sync.WaitGroup) {
	...
	vrfm.runInternal(stopCh, doneWg, subscribe)
}

type subscribeFn func() (bool, chan netlink.LinkUpdate, error)

func (vrfm *Controller) runInternal(stopChan <-chan struct{}, doneWg *sync.WaitGroup, subscribe subscribeFn) {
	doneWg.Add(1)
	go func() {
		defer doneWg.Done()
		...
		for {
			...
			case a, ok := <-addrChan:
				...
				if err = vrfm.syncLink(a.Link); err != nil {
