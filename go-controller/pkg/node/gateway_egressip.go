...
func (e markIPs) insert(mark util.EgressIPMark, ip net.IP) {
	if ip == nil || len(ip) == 0 || !mark.IsAvailable() || !mark.IsValid() {
...
func (e markIPs) contains(mark util.EgressIPMark, ip net.IP) bool {
	if ip == nil || len(ip) == 0 || !mark.IsAvailable() || !mark.IsValid() {
		klog.Errorf("Invalid IP and/or pkt mark arguments therefore not checking EgressIP config cache: mark %v, IP %v", mark, ip)
		return false
	}
	var existingIP string
	var isPresent bool
	if ip.To4() != nil {
		existingIP, isPresent = e.v4[mark.ToInt()]
	} else if ip.To16() != nil {
		existingIP, isPresent = e.v6[mark.ToInt()]
	}
	return isPresent && existingIP == ip.String()
}
...
func (mic *markIPsCache) isMarkIPPresent(pktMark util.EgressIPMark, ip net.IP) bool {
	mic.mu.Lock()
	defer mic.mu.Unlock()
	return mic.cache.contains(pktMark, ip)
}
