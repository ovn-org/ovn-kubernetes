...
func (nc *DefaultNodeNetworkController) initGatewayPreStart(subnets []*net.IPNet, nodeAnnotator kube.Annotator,
	...
	klog.Info("RICCARDO initGatewayPreStart Initializing Gateway Functionality")
	var err error
	...
	klog.Infof("RICCARDO initGatewayPreStart Waiting for gateway and management port readiness...")
	start := time.Now()
	if err := waiter.Wait(); err != nil {
		return nil, err
	}
	klog.Infof("RICCARDO initGatewayPreStart Gateway and management port readiness took %v", time.Since(start))

	return gw, nil
}

// initGatewayMainStart finishes the gateway initialization for the node.
func (nc *DefaultNodeNetworkController) initGatewayMainStart(gw *gateway, waiter *startupWaiter) error {
	klog.Info("RICCARDO initGatewayMainStart Initializing Gateway Functionality")
	...
	waiter.AddWait(readyGwFunc, initGwFunc)
	nc.Gateway = gw
	klog.Info("RICCARDO initGatewayMainStart Initializing Gateway Functionality DONE")
