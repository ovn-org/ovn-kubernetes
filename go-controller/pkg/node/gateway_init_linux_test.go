...
var _ = Describe("Gateway Init Operations", func() {
	...
	Context("for localnet operations", func() {
		...
		It("sets up a IPv4 localnet gateway", func() {
			subnets = append(subnets, ovntest.MustParseIPNet(v4NodeSubnet))
			...
			localNetInterfaceTest(app, testNS, subnets, brNextHopCIDRs, ipts, expectedIpTableRules)
		})

		It("sets up a IPv6 localnet gateway", func() {
			subnets = append(subnets, ovntest.MustParseIPNet(v6NodeSubnet))
		...
		It("sets up a dual stack localnet gateway", func() {
			//Skip("Skipping the test for now")
			subnets = append(subnets, ovntest.MustParseIPNet(v4NodeSubnet))
			subnets = append(subnets, ovntest.MustParseIPNet(v6NodeSubnet))
