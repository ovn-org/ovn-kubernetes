...
func shareGatewayInterfaceTest(app *cli.App, testNS ns.NetNS,
	...
	app.Action = func(ctx *cli.Context) error {
		...
		fexec.AddFakeCmdsNoOutputNoError([]string{
			...
			"ovs-ofctl add-flow breth0 cookie=0xdeff105, priority=1000, table=0, in_port=7, tcp, ip_dst=192.168.1.10, tp_dst=6443, actions=LOCAL",
			"ovs-ofctl add-flow breth0 cookie=0xdeff105, priority=1000, table=0, in_port=LOCAL, tcp, ip_src=192.168.1.10, tp_src=6443, actions=output:7",
			"ovs-ofctl add-flow breth0 cookie=0xdeff105, priority=1000, table=0, in_port=7, tcp, ip_dst=192.168.1.10, tp_dst=2379, actions=LOCAL",
			"ovs-ofctl add-flow breth0 cookie=0xdeff105, priority=1000, table=0, in_port=LOCAL, tcp, ip_src=192.168.1.10, tp_src=2379, actions=output:7",
			"ovs-ofctl add-flow breth0 cookie=0xdeff105, priority=1000, table=0, in_port=7, tcp, ip_dst=192.168.1.10, tp_dst=2380, actions=LOCAL",
			"ovs-ofctl add-flow breth0 cookie=0xdeff105, priority=1000, table=0, in_port=LOCAL, tcp, ip_src=192.168.1.10, tp_src=2380, actions=output:7",
			...
			"ovs-appctl --timeout=15 fdb/flush breth0",
		})
		// nodePortWatcher()
		fexec.AddFakeCmd(&ovntest.ExpectedCmd{
