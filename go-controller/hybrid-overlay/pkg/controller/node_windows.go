...
func (n *NodeController) Add(node *kapi.Node) {
	...
	if !ok {
		klog.V(5).Infof("Cannot add node '%s' as the k8s.ovn.org/hybrid-overlay-hostsubnet annotation is missing on that node!", node.Name)
		return
	}
	...
	if node.Status.NodeInfo.MachineID != n.thisNode.Status.NodeInfo.MachineID {
		...
		if cidr != nil && nodeIP != nil && drMAC != nil {
			...
			klog.Infof("Adding a remote subnet route for CIDR '%s' (remote node address: %s, distributed router MAC: %s, VNI: %v).", cidr.String(), nodeIP.String(), drMAC.String(), types.HybridOverlayVNI)
...
func (n *NodeController) Delete(node *kapi.Node) {
	...
	if err != nil {
		if _, isNotExist := err.(hcn.NetworkNotFoundError); !isNotExist {
			klog.Errorf("Couldn't retreive network with ID '%s' on node '%s'", n.networkID, node.Name)
		} else {
			klog.Infof("No need to cleanup remote subnet for network with ID '%s' towards node '%s' as it doesn't exist", n.networkID, node.Name)
		}
		return
	}
	...
	if !ok {
		klog.Errorf("Can't retreive the host subnet from the '%s' node's annotations", node.Name)
		return
	}

	err = RemoveRemoteSubnetPolicy(network, nodeSubnet)
	if err != nil {
		klog.Errorf("Error removing subnet policy '%s' node's annotations from network '%s' on node '%s'. Error: %v", nodeSubnet, n.networkID, node.Name, err)
		return
	}
...
func (n *NodeController) InitSelf() {
	...
	if !ok {
		klog.Errorf("Couldn't retreive the host subnet from the '%s' node's annotations", n.thisNode.Name)
		return
	}
	...
	if baseNetwork == nil {
		...
		if err != nil {
			klog.Errorf("Unable to generate a schema to create a base overlay network, error: %v", err)
			return
		}
		...
		if err != nil {
			klog.Errorf("Unable to create the base overlay network, error: %v", err)
			return
		}
		...
		if err != nil {
			klog.Errorf("Unable to refresh the persistent IP routes, error: %v", err)
			return
		}
	}
	...
	if network == nil {
		...
		klog.Infof("Creating overlay network '%s' (address prefix %v) with gateway address: %v. Use automatic DNS: %t.", networkName, addressPrefix, gatewayAddress, useAutomaticDns)

		// Retreive the network schema object
		var networkSchema *hcn.HostComputeNetwork
		networkSchema, err = networkInfo.GetHostComputeNetworkConfig()
		if err != nil {
			klog.Errorf("Unable to generate a schema to create an overlay network, error: %v", err)
			return
		}
		...
		if err != nil {
			klog.Errorf("Unable to create the overlay network, error: %v", err)
			return
		}

		err = AddHostRoutePolicy(network)
		if err != nil {
			klog.Errorf("Unable to add host route policy, error: %v", err)
			return
		}
	} else {
		klog.Infof("Reusing existing overlay network '%s' (address prefix %v) with gateway address: %v.", networkName, addressPrefix, gatewayAddress)
	...
	for _, policy := range network.Policies {
		if policy.Type == hcn.DrMacAddress {
			...
			if err != nil {
				klog.Errorf("Unable to unmarshall the DRMAC policy setting, error: %v", err)
				return
			}

			if len(policySettings.Address) == 0 {
				klog.Errorf("Error creating the network: no DRMAC address")
				return
			}
	...
	if err != nil {
		klog.Errorf("Error in initializing/fetching nodes: %v", err)
		return
	}
...
func (n *NodeController) UninitSelf() {
	klog.Infof("Removing overlay network '%s' (ID: %v) from local node '%s'", networkName, n.networkID, n.thisNode.Name)

	// Remove existing nodes
	nodes, err := n.kube.GetNodes()
	if err != nil {
		klog.Errorf("Error in uninitializing/fetching nodes: %v", err)
		return
	}
