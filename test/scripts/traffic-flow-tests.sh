...
export TRAFFIC_FLOW_TESTS_COMMIT="6a3a529012a514887a7032d99aed72f1a782b0dc"
...
process_test_results() {
    ...
    python evaluator.py eval-config.yaml ${result_file} ${result_file}.evaluated || error_exit "Evaluation failed."
    ./print_results.py ${result_file}.evaluated || error_exit "Results evaluation failed."
}
...
setup() {
    ...
    # TODO(FF): Come up with better thresholds for eval-config file
    local EVAL_CONFIG_FILE="eval-config.yaml"
    sed -i 's/\(threshold: *\).*/\11/' "$EVAL_CONFIG_FILE"
...
run() {
    ...
    time python main.py config.yaml -o "$OUTPUT_BASE" || error_exit "Test execution FAILED."
