...
func reconfigureVLANAccessPort(ovsPods []v1.Pod, portName string, oldVLANID, newVLANID int) error {
	for _, ovsPod := range ovsPods {
		if err := ovsRemoveVLANAccessPort(ovsPod.Name, bridgeName, portName, oldVLANID); err != nil {
...
func ovsRemoveVLANAccessPort(ovsNodeName string, bridgeName string, portName string, vlanID int) error {
	cmd := []string{
		"kubectl", "-n", ovnNamespace, "exec", ovsNodeName, "--",
