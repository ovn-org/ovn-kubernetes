...
func createPod(f *framework.Framework, podName, nodeSelector, namespace string, command []string, labels map[string]string, options ...func(*v1.Pod)) *v1.Pod {
	...
	}, 30*time.Second, 1*time.Second).Should(gomega.Not(gomega.Equal("")), "Failed to get pod ip")
	return res
}
...
var _ = ginkgo.Describe("e2e multiple ecmp external gateway validation", func() {
	const (
		...
		pingContainer1     string = "gw-ecmp-test-container1"
		pingContainer2     string = "gw-ecmp-test-container2"
		ciNetworkName      string = "kind"
	...
	ginkgo.AfterEach(func() {
		// tear down the containers simulating the gateways
		if cid, _ := runCommand("docker", "ps", "-qaf", fmt.Sprintf("name=%s", pingContainer1)); cid != "" {
			if _, err := runCommand("docker", "rm", "-f", pingContainer1); err != nil {
				framework.Logf("failed to delete the gateway test container %s %v", pingContainer1, err)
			}
		}
		if cid, _ := runCommand("docker", "ps", "-qaf", fmt.Sprintf("name=%s", pingContainer2)); cid != "" {
			if _, err := runCommand("docker", "rm", "-f", pingContainer2); err != nil {
				framework.Logf("failed to delete the gateway test container %s %v", pingContainer2, err)
			}
		}
	})
	...
	ginkgo.It("Should validate connectivity to multiple external gateways for an ECMP scenario", func() {
		...
		exPingIP1 := createClusterExternalContainer(pingContainer1, "centos/tools", []string{"-itd", "--privileged", "--network", ciNetworkName}, []string{})
		exPingIP2 := createClusterExternalContainer(pingContainer2, "centos/tools", []string{"-itd", "--privileged", "--network", ciNetworkName}, []string{})
		...
		annotateArgs := []string{
			...
			"namespace",
			f.Namespace.Name,
			fmt.Sprintf("k8s.ovn.org/routing-external-gws=%s,%s", exPingIP1, exPingIP2),
		}
		framework.Logf("Annotating the external gateway test namespace to container gateways: %s, %s", exPingIP1, exPingIP2)
		framework.RunKubectlOrDie(f.Namespace.Name, annotateArgs...)

		targetIPs := make([]string, 0)
		for lastOctet := 1; lastOctet <= ecmpRetry; lastOctet++ {
			destIP := fmt.Sprintf("%s%d", exGWRemoteIpPrefix, lastOctet)
		...
		framework.ExpectNoError(err)
		node := nodes.Items[0]
		nodeIP := node.Status.Addresses[0]
		framework.Logf("the pod side node is %s and the source node ip is %s", node.Name, nodeIP)

		// Create the pod that will be used as the source for the connectivity test
		clientPod := createGenericPod(f, srcPingPodName, node.Name, f.Namespace.Name, command)
		...
		for _, containerName := range []string{pingContainer1, pingContainer2} {
			for _, address := range targetIPs {
			...
			_, err = runCommand("docker", "exec", containerName, "ip", "route", "add", clientIP, "via", nodeIP.Address)
		...
		checkPingOnContainer := func(container string) error {
			_, err = runCommand("docker", "exec", container, "timeout", testTimeout, "tcpdump", "-c", "1", "icmp")
			if err != nil {
				return err
			}
		...
		g.Go(func() error { return checkPingOnContainer(pingContainer1) })
		g.Go(func() error { return checkPingOnContainer(pingContainer2) })
