...
var _ = ginkgo.Describe("Services", func() {
	...
	ginkgo.It("Creates a service with session-affinity, and ensures it works after backend deletion", func() {
		...
		hosts := affinityCheckFromPod(execPod, svc.Spec.ClusterIP, int(svc.Spec.Ports[0].Port))
...
func affinityCheckFromPod(execPod *v1.Pod, serviceIP string, servicePort int) []string {
