...
var _ = Describe("Kubevirt Virtual Machines", func() {
	var (
		...
		isDualStack = func() bool {
			...
			for _, a := range nodeList.Items[0].Status.Addresses {
	...
	var (
		...
		waitVirtualMachineAddressesFromStatus = func(vmi *kubevirtv1.VirtualMachineInstance, expectedNumberOfAddresses int) []string {
...
`, tcpServerPort)
		...
		startTestPods = func(annotations map[string]string, checkPodFn func(g Gomega, pod *corev1.Pod)) {
			...
			for _, selectedNode := range selectedNodes {
				...
				httpServerTestPod.Annotations = annotations
			...
			for _, httpServerTestPod := range httpServerTestPods {
				Eventually(func(g Gomega) {
					...
					g.Expect(err).ToNot(HaveOccurred())
					checkPodFn(g, httpServerTestPod)
				}).
	...
	Context("with default pod network", func() {
		...
		BeforeEach(func() {
			...
			startTestPods(map[string]string{}, checkPodHasIPAtStatus)
...
`
		...
		DescribeTable("should keep ip", func(td testData) {
			...
			startTestPods(map[string]string{
			...
			Expect(kubevirt.LoginToFedora(vmi, "core", "fedora")).To(Succeed(), step)
			expectedAddreses = waitVirtualMachineAddressesFromStatus(vmi, 2 /*two addresses, dual stack*/)
			...
			Expect(kubevirt.LoginToFedora(vmi, "core", "fedora")).To(Succeed(), step)
			obtainedAddresses := waitVirtualMachineAddressesFromStatus(vmi, 2 /*two addresses, dual stack*/)
