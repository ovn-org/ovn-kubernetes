...
var _ = Describe("Kubevirt Virtual Machines", func() {
	...
	var (
		...
		checkConnectivity = func(tcpProbeConns []net.Conn) {
		...
		composeVM = func(labels map[string]string) (*kvv1.VirtualMachine, error) {
			butaneIPv4 := `
variant: fcos
version: 1.4.0
passwd:
...
`
	...
	DescribeTable("when live migrated", func(migrationMode kvv1.MigrationMode) {
		...
		By("Check connectivity before live migration")
		checkConnectivity(tcpProbeConns)
		...
		FEntry("with pre-copy should keep connectivity", kvv1.MigrationPreCopy),
