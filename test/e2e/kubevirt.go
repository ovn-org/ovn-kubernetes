...
var _ = Describe("Kubevirt Virtual Machines", func() {
	...
	var (
		...
		*/
		...
		checkPodHasIPsAtOVNAnnotation = func(nadName string, expectedNumberOfAddresses int) func(Gomega, *corev1.Pod) {
			return func(g Gomega, pod *corev1.Pod) {
				GinkgoHelper()
				ovnPodAnnotation, err := util.UnmarshalPodAnnotation(pod.Annotations, nadName)
				g.Expect(err).ToNot(HaveOccurred())
				g.Expect(ovnPodAnnotation.IPs).To(HaveLen(expectedNumberOfAddresses))
			}
		}
...
`
		...
		DescribeTable("should keep ip", func(td testData) {
			...
			if td.role != "primary" {
				httpServerPodCondition = checkPodHasIPsAtNetwork(networkName, 2 /*expectedNumberOfAddresses*/)
			} else {
				httpServerPodCondition = checkPodHasIPsAtOVNAnnotation(networkName, 2 /*expectedNumberOfAddresses*/)
			}
