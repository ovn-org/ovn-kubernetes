...
func changeNodeZone(node *v1.Node, zone string, cs clientset.Interface) error {
	...
	err = restartOVNKubePod(cs, "ovn-kubernetes", node.Name, "ovnkube-node")
	framework.ExpectNoError(err)
...
var _ = ginkgo.Describe("Multi node zones interconnect", func() {
	...
	ginkgo.It("Pod interconnectivity", func() {
		// Create a server pod on zone - az1 and node - ovn-worker2
		...
		// Create a client on zone - az2 and node - ovn-worker4
		cmd = []string{}
