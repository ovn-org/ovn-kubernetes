...
func testMulticastIGMPQuery(f *framework.Framework, clientNodeInfo, serverNodeInfo nodeInfo) {
	...
	// The multicast listener pod joins the multicast group (-B 224.1.1.1), UDP (-u), for (-t 5) seconds, reporting every (-i 5) seconds
	...
	if err != nil {
		framework.Failf("failed to join multicast group: %v", err)
	}
	...
	}, 2*time.Minute, 5*time.Second).Should(gomega.ContainSubstring("igmp"), "Failed to retrieve multicast IGMP query within the expected time")
}
