...
var _ = ginkgo.Describe("e2e egress IP validation", func() {
	...
	configNetworkAndTargetForMultiNIC := func(subnet string, nodesToAttachNet []string, v6 bool) {
		// configure and add additional network to worker containers for EIP multi NIC feature
		createNetwork(secondaryNetworkName, subnet)
		...
		if v6 {
			targetSecondaryNode.nodeIP = v6Addr
		} else {
			targetSecondaryNode.nodeIP = v4Addr
		}
	}
	...
	ginkgo.BeforeEach(func() {
		...
		if isV6 {
			...
			// configure and add additional network to worker containers for EIP multi NIC feature
			configNetworkAndTargetForMultiNIC(secondaryIPV6Subnet, []string{egress1Node.name, egress2Node.name}, isV6)
		} else {
			...
			// configure and add additional network to worker containers for EIP multi NIC feature
			configNetworkAndTargetForMultiNIC(secondaryIPV4Subnet, []string{egress1Node.name, egress2Node.name}, isV6)
		}
...
`)
	...
	*/
	ginkgo.It("[OVN managed network] Should validate the egress IP SNAT functionality for stateful-sets", func() {
...
`)
	...
	*/
	ginkgo.It("[OVN managed network] Should validate egress IP logic when one pod is managed by more than one egressIP object", func() {
...
`)
	...
	*/
	ginkgo.It("[OVN managed network] Should re-assign egress IPs when node readiness / reachability goes down/up", func() {
...
`)
	...
	*/
	ginkgo.It("[OVN managed network] Should validate the egress IP functionality against remote hosts with egress firewall applied", func() {
...
`)
	...
	/* This test does the following:
	   ...
	   9. Check connectivity from pod that isn't selected by EgressIP anymore to an external "node" on the OVN managed network and verify that the src IP is the node IP.
	   10. Check connectivity does not occur from pod that isn't selected by EgressIP anymore to an external "node" on the non-OVN managed network.
	   11. Update the unselected pod to be selected by the EgressIP
	   12. Check connectivity from both pods to an external "node" hosted on the non-OVN managed network and verify the expected src IPs
	   13. Set one node as unavailable for egress
	   14. Check that the status is of length one
	   15. Check that correct Egress IP is assigned
	   16. Check connectivity from a pod to an external "node" on the non-OVN managed network and verify that the src IP is the remaining egress IP
	   17. Set the other node as unavailable for egress
	   18. Check connectivity from a pod to an external "node" on the OVN managed network and verify that the src IP is the node IP
	   19. Check that the status is of length zero
	   20. Set a node back as available for egress
	   21. Check that the status is of length one
	   22. Check that correct Egress IP is assigned
	   23. Check connectivity from a pod to an external "node" on the non-OVN managed network and verify that the src IP is the remaining egress IP
	   24. Set the other node back as available for egress
	   25. Check that the status is of length two
	   26. Check that correct Egress IP is assigned
	   27. Check connectivity from the other pod to an external "node" on the non-OVN managed network and verify the expected src IPs
	*/
	ginkgo.It("[non-OVN managed network] Using different methods to disable a node or pod availability for egress", func() {
		...
		egressIPIP1 := "10.10.10.100"
		egressIPIP2 := "10.10.10.200"
		egressIPConfig := fmt.Sprintf(`apiVersion: k8s.ovn.org/v1
kind: EgressIP
metadata:
    name: ` + egressIPName + `
...
`)
		...
		ginkgo.By("10. Check connectivity does not occur from pod that isn't selected by EgressIP anymore to an external \"node\" " +
			"on the non-OVN managed network.")
		err = wait.PollImmediate(retryInterval, retryTimeout, targetExternalContainerAndTest(targetSecondaryNode, pod2Name,
			podNamespace.Name, false, []string{}))
		framework.ExpectNoError(err, "10. Check connectivity does not occur from pod that isn't selected by EgressIP "+
			"anymore to an external \"node\" on the non-OVN managed network, failed, err: %v", err)

		ginkgo.By("11. Update the unselected pod to be selected by the EgressIP")
		pod2 = getPod(f, pod2Name)
		pod2.Labels = podEgressLabel
		updatePod(f, pod2)

		ginkgo.By("12. Check connectivity from both pods to an external \"node\" hosted on the non-OVN managed network " +
			"and verify the expected IPs")
		...
		framework.ExpectNoError(err, "Step 12. Check connectivity from pod (%s/%s) to an external container attached to "+
			"a network that isn't OVN managed and verify that the src IP is the expected egressIP, failed, err: %v", podNamespace.Name, pod1Name, err)
		err = wait.PollImmediate(retryInterval, retryTimeout, targetExternalContainerAndTest(targetSecondaryNode, pod2Name, podNamespace.Name,
			true, []string{egressIPIP1, egressIPIP2}))
		framework.ExpectNoError(err, "Step 12. Check connectivity from pod (%s/%s) to an external container attached to "+
			"a network that isn't OVN managed and verify that the src IP is the expected egressIP, failed, err: %v", podNamespace.Name, pod2Name, err)

		ginkgo.By("13. Set one node as unavailable for egress")
		egressNodeAvailabilityHandler.Disable(egress1Node.name)

		ginkgo.By("14. Check that the status is of length one")
		statuses = verifyEgressIPStatusLengthEquals(1, nil)

		ginkgo.By("15. Check that correct Egress IP is assigned")
		...
		ginkgo.By("16. Check connectivity from a pod to an external \"node\" on the non-OVN managed network and " +
			"verify that the IP is the remaining egress IP")
		err = wait.PollImmediate(retryInterval, retryTimeout, targetExternalContainerAndTest(targetSecondaryNode, pod1Name,
			podNamespace.Name, true, []string{statuses[0].EgressIP}))
		framework.ExpectNoError(err, "16. Check connectivity from a pod to an external \"node\" on the non-OVN managed"+
			" network and verify that the IP is the remaining egress IP, failed, err: %v", err)

		ginkgo.By("17. Set the other node as unavailable for egress")
		egressNodeAvailabilityHandler.Disable(egress2Node.name)

		ginkgo.By("18. Check connectivity from a pod to an external \"node\" on the OVN managed network and " +
		...
		framework.ExpectNoError(err, "18. Check connectivity from a pod to an external \"node\" on the OVN managed network "+
		...
		ginkgo.By("20. Set a node back as available for egress")
		egressNodeAvailabilityHandler.Enable(egress1Node.name)

		ginkgo.By("21. Check that the status is of length one")
		statuses = verifyEgressIPStatusLengthEquals(1, nil)

		ginkgo.By("22. Check that correct Egress IP is assigned")
		gomega.Expect(verifyEgressIPStatusContainsIPs(statuses, []string{egressIPIP1}) || verifyEgressIPStatusContainsIPs(statuses, []string{egressIPIP2})).Should(gomega.BeTrue())

		ginkgo.By("23. Check connectivity from a pod to an external \"node\" on the non-OVN managed network and verify " +
			"that the IP is the remaining egress IP")
		err = wait.PollImmediate(retryInterval, retryTimeout, targetExternalContainerAndTest(targetSecondaryNode, pod1Name,
			podNamespace.Name, true, []string{statuses[0].EgressIP}))
		framework.ExpectNoError(err, "23. Check connectivity from a pod (%s/%s) to an external \"node\" on the non-OVN managed network and verify "+
		...
		ginkgo.By("27. Check connectivity from the other pod to an external \"node\" on the non-OVN managed network and verify the expected IPs")
		err = wait.PollImmediate(retryInterval, retryTimeout, targetExternalContainerAndTest(targetSecondaryNode, pod2Name,
			podNamespace.Name, true, []string{egressIPIP1, egressIPIP2}))
		framework.ExpectNoError(err, "27. Check connectivity from the other pod (%s/%s) to an external \"node\" on the "+
			"non-OVN managed network and verify the expected IPs, failed, err: %v", podNamespace, pod2Name, err)
	})
	...
	/* This test does the following:
	   ...
	   10. Check connectivity from pod that isn't selected by EgressIP anymore to an external "node" on the OVN managed network and verify that the IP is the node IP.
	   11. Check connectivity does not occur from pod that isn't selected by EgressIP anymore to an external "node" on the non-OVN managed network.
	   12. Update the unselected pod to be selected by the EgressIP
	   13. Check connectivity from both pods to an external "node" hosted on the OVN managed network and the src IP is the expected egressIP
	   14. Check connectivity from both pods to an external "node" hosted on the non-OVN managed network and the src IP is the expected egressIP
	   15. Set the node hosting the OVN managed egress IP as unavailable
	   16. Check that the status is of length one
	   17. Check that correct Egress IP is assigned
	   18. Check connectivity from both pods to an external "node" on the non-OVN managed network and verify the src IP is the expected egressIP
	   19. Set the other node, which is hosting the non-OVN managed egress IP as unavailable for egress
	   20. Check that the status is of length zero
	   21. Check connectivity from both pods to an external "node" on the OVN managed network and verify that the src IP is the node IPs
	   22. Set a node (hosting non-OVN managed EgressIP) back as available for egress
	   23. Check that the status is of length one
	   24. Check that correct Egress IP is assigned
	   25. Set the other node back as available for egress
	   26. Check that the status is of length two
	   27. Check that correct Egress IPs are assigned
	   28. Check connectivity both pods to an external "node" on the OVN managed network and verify the src IP is the expected egressIP
	   29. Check connectivity both pods to an external "node" on the non-OVN managed network and verify the src IP is the expected egressIP
	*/
	ginkgo.It("[OVN+non-OVN managed networks] Using different methods to disable a node or pod availability for egress", func() {
		if utilnet.IsIPv6(net.ParseIP(egress1Node.nodeIP)) {
			ginkgo.Skip("Node does not have IPv4 address")
		}
		egressNodeAvailabilityHandler := egressNodeAvailabilityHandlerViaLabel{f}
...
`)
		...
		ginkgo.By("11. Check connectivity does not occur from pod that isn't selected by Egress IP anymore to an external \"node\" " +
			"on the non-OVN managed network.")
		err = wait.PollImmediate(retryInterval, retryTimeout, targetExternalContainerAndTest(targetSecondaryNode, pod2Name,
			podNamespace.Name, false, []string{}))
		framework.ExpectNoError(err, "10. Check connectivity does not occur from pod that isn't selected by Egress IP "+
			"anymore to an external \"node\" on the non-OVN managed network, failed, err: %v", err)

		ginkgo.By("12. Update the unselected pod to be selected by the Egress IP")
		pod2 = getPod(f, pod2Name)
		pod2.Labels = podEgressLabel
		updatePod(f, pod2)

		ginkgo.By("13. Check connectivity from both pods to an external \"node\" hosted on the OVN managed network " +
			"and verify that the expected IP")
		err = wait.PollImmediate(retryInterval, retryTimeout, targetExternalContainerAndTest(targetNode, pod1Name, podNamespace.Name,
			true, []string{egressIPOVNManaged}))
		framework.ExpectNoError(err, "Step 13. Check connectivity from pod (%s/%s) to an external container attached to "+
			"a network that is OVN managed and verify that the src IP is the expected egress IP, failed, err: %v", podNamespace.Name, pod1Name, err)
		err = wait.PollImmediate(retryInterval, retryTimeout, targetExternalContainerAndTest(targetNode, pod2Name, podNamespace.Name,
			true, []string{egressIPOVNManaged}))
		framework.ExpectNoError(err, "Step 13. Check connectivity from pod (%s/%s) to an external container attached to "+
			"a network that is OVN managed and verify that the src IP is the expected egress IP, failed, err: %v", podNamespace.Name, pod2Name, err)

		ginkgo.By("14. Check connectivity from both pods to an external \"node\" hosted on non-OVN managed network " +
			"and verify that the expected IP")
		err = wait.PollImmediate(retryInterval, retryTimeout, targetExternalContainerAndTest(targetSecondaryNode, pod1Name, podNamespace.Name,
			true, []string{egressIPNonOVNManaged}))
		framework.ExpectNoError(err, "Step 14. Check connectivity from pod (%s/%s) to an external container attached to "+
			"a network that isn't OVN managed and verify that the src IP is the expected egress IP, failed, err: %v", podNamespace.Name, pod1Name, err)
		err = wait.PollImmediate(retryInterval, retryTimeout, targetExternalContainerAndTest(targetSecondaryNode, pod2Name, podNamespace.Name,
			true, []string{egressIPNonOVNManaged}))
		framework.ExpectNoError(err, "Step 14. Check connectivity from pod (%s/%s) to an external container attached to "+
			"a network that isn't OVN managed and verify that the src IP is the expected egress IP, failed, err: %v", podNamespace.Name, pod2Name, err)

		ginkgo.By("15. Set the node hosting the OVN managed egress IP as unavailable")
		var nodeNameHostingOVNManagedEIP, nodeNameHostingNonOVNManagedEIP string
		for _, status := range statuses {
		...
		ginkgo.By("16. Check that the status is of length one")
		statuses = verifyEgressIPStatusLengthEquals(1, nil)

		ginkgo.By("17. Check that correct Egress IP is assigned")
		gomega.Expect(verifyEgressIPStatusContainsIPs(statuses, []string{egressIPNonOVNManaged})).Should(gomega.BeTrue())

		ginkgo.By("18. Check connectivity from both pods to an external \"node\" on the non-OVN managed network and " +
			"verify that the src IP is the expected egress IP")
		err = wait.PollImmediate(retryInterval, retryTimeout, targetExternalContainerAndTest(targetSecondaryNode, pod1Name,
			podNamespace.Name, true, []string{statuses[0].EgressIP}))
		framework.ExpectNoError(err, "18. Check connectivity from both pods (%s/%s) to an external \"node\" on the non-OVN managed"+
		...
		ginkgo.By("19. Set the other node, which is hosting the non-OVN managed egress IP as unavailable for egress")
		egressNodeAvailabilityHandler.Disable(nodeNameHostingNonOVNManagedEIP)

		ginkgo.By("20. Check that the status is of length zero")
		statuses = verifyEgressIPStatusLengthEquals(0, nil)

		ginkgo.By("21. Check connectivity from both pods to an external \"node\" on the OVN managed network and verify that the src IP is the node IPs")
		err = wait.PollImmediate(retryInterval, retryTimeout, targetExternalContainerAndTest(targetNode, pod1Name,
			podNamespace.Name, true, []string{pod1Node.nodeIP}))
		framework.ExpectNoError(err, "21. Check connectivity from both pods (%s/%s) to an external \"node\" on the "+
			"OVN managed network and verify that the src IP is the node IP %s, failed: %v", podNamespace, pod1Name, pod1Node.nodeIP, err)
		err = wait.PollImmediate(retryInterval, retryTimeout, targetExternalContainerAndTest(targetNode, pod2Name,
			podNamespace.Name, true, []string{pod2Node.nodeIP}))
		framework.ExpectNoError(err, "21. Check connectivity from both pods (%s/%s) to an external \"node\" on the "+
			"OVN managed network and verify that the src IP is the node IP %s, failed: %v", podNamespace, pod2Name, pod2Node.nodeIP, err)

		ginkgo.By("22. Set a node (hosting non-OVN managed Egress IP) back as available for egress")
		egressNodeAvailabilityHandler.Enable(nodeNameHostingNonOVNManagedEIP)

		ginkgo.By("23. Check that the status is of length one")
		statuses = verifyEgressIPStatusLengthEquals(1, nil)

		ginkgo.By("24. Check that correct Egress IP is assigned")
		gomega.Expect(verifyEgressIPStatusContainsIPs(statuses, []string{egressIPNonOVNManaged}) || verifyEgressIPStatusContainsIPs(statuses, []string{egressIPOVNManaged})).Should(gomega.BeTrue())

		ginkgo.By("25. Set the other node back as available for egress")
		egressNodeAvailabilityHandler.Enable(nodeNameHostingOVNManagedEIP)

		ginkgo.By("26. Check that the status is of length two")
		statuses = verifyEgressIPStatusLengthEquals(2, nil)

		ginkgo.By("27. Check that correct Egress IPs are assigned")
		gomega.Expect(verifyEgressIPStatusContainsIPs(statuses, []string{egressIPOVNManaged, egressIPNonOVNManaged})).Should(gomega.BeTrue())

		ginkgo.By("28. Check connectivity from both pods to an external \"node\" on the OVN managed network and verify the src IP is the expected egress IP")
		err = wait.PollImmediate(retryInterval, retryTimeout, targetExternalContainerAndTest(targetNode, pod1Name,
			podNamespace.Name, true, []string{egressIPOVNManaged}))
		framework.ExpectNoError(err, "Step 28. Check connectivity from pod (%s/%s) to an external container attached to "+
			"a network that is OVN managed and verify that the src IP is the expected egress IP, failed: %v", podNamespace.Name, pod1Name, err)
		err = wait.PollImmediate(retryInterval, retryTimeout, targetExternalContainerAndTest(targetNode, pod2Name,
			podNamespace.Name, true, []string{egressIPOVNManaged}))
		framework.ExpectNoError(err, "Step 28. Check connectivity from pod (%s/%s) to an external container attached to "+
			"a network that is OVN managed and verify that the src IP is the expected egress IP, failed: %v", podNamespace.Name, pod2Name, err)

		ginkgo.By("29. Check connectivity both pods to an external \"node\" on the non-OVN managed network and verify the src IP is the expected egress IP")
		err = wait.PollImmediate(retryInterval, retryTimeout, targetExternalContainerAndTest(targetSecondaryNode, pod1Name,
			podNamespace.Name, true, []string{egressIPNonOVNManaged}))
		framework.ExpectNoError(err, "Step 29. Check connectivity from pod (%s/%s) to an external container attached to "+
			"a network that is non-OVN managed and verify that the src IP is the expected egress IP, failed: %v", podNamespace.Name, pod1Name, err)
		err = wait.PollImmediate(retryInterval, retryTimeout, targetExternalContainerAndTest(targetSecondaryNode, pod2Name,
			podNamespace.Name, true, []string{egressIPNonOVNManaged}))
		framework.ExpectNoError(err, "Step 29. Check connectivity from pod (%s/%s) to an external container attached to "+
			"a network that is non-OVN managed and verify that the src IP is the expected egress IP, failed: %v",
			podNamespace.Name, pod2Name, err)
	})
})
