		...
		Context("localnet OVN-K secondary network", func() {
			...
			Context("with a service running on the underlay", func() {
				...
				It("correctly sets the MTU on the pod", func() {
					...
					Eventually(getSecondaryInterfaceMTU).WithArguments(clientPodConfig).Should(Equal(expectedOriginalMTU), "pod's MTU is properly configured")
				})
				...
				Context("and networkAttachmentDefinition is modified", func() {
					...
					BeforeEach(func() {
						...
						Expect(patchNADSpec(nadClient, netConfig.name, netConfig.namespace, p)).NotTo(Succeed())
					})
					...
					It("can no longer communicate over a localnet secondary network from pod to the underlay service", func() {
						...
						Eventually(connectToServer).WithArguments(clientPodConfig, underlayServiceIP, servicePort).ShouldNot(Succeed())
					})

					Context("and the service connected to the underlay is reconfigured to connect to the new VLAN-ID", func() {
						BeforeEach(func() {
							Expect(reconfigureVLANAccessPort(nodes, secondaryInterfaceName, newLocalnetVLANID)).To(Succeed())
						})
