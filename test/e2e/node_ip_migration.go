...
`
	...
	BeforeEach(func() {
		...
		workerNodeIPs[4], workerNodeIPs[6] = getNodeInternalAddrs(&workerNode)
		secondaryWorkerNodeIPs = make(map[int]string)
		secondaryWorkerNodeIPs[4], secondaryWorkerNodeIPs[6] = getNodeInternalAddrs(&secondaryWorkerNode)
		...
		When(fmt.Sprintf("the node IPv%d address is updated", ipAddrFamily), func() {
			...
			JustAfterEach(func() {
				if rollbackNeeded {
					...
					// It takes ovn-controller 200 seconds to reconnect to the SBDB after an IP address change.
					// See here for more details: https://github.com/ovn-org/ovn-kubernetes/pull/3131#issuecomment-1351871875
			...
			Context("when no EgressIPs are configured", func() {
				for _, updateKubeletFirst := range []bool{true, false} {
					...
					It(fmt.Sprintf("makes sure that the cluster is still operational (%s)",
						...
						func() {
							By(fmt.Sprintf("Migrating worker node %s from IP address %s to IP address %s (inverted)",
...
// getNodeInternalAddrs returns the first IPv4 and/or IPv6 InternalIP defined for the node. Node IPs are ordered,
...
func getNodeInternalAddrs(node *v1.Node) (string, string) {
