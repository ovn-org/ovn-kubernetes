...
- A localnet topology allows connections to the physical network, but requires additional configuration of the
  underlying Open vSwitch (OVS) bridge on cluster nodes.
...
Multihoming enables the workloads users (Pods and VMs) to send/receive traffic on network interfaces separate from the
primary K8s network, without using complicated routing on the underlying physical network.
It allows interconnecting the workloads through the OVN-Kubernetes logical switches to east-west workloads and to
physical networks.

For VM workloads, Multihoming can ensure that VMs retain their IP addresses even when they move across nodes, enhancing
workload mobility and minimizing disruptions.
...
- cluster-wide overlay network on layer 2:
  In this case example, two VMs are connected over a secondary-network, where VMA accesses a database deployed on VMB.
  The IPs are assigned via a Pod running DHCP, connected to the same layer 2 network.
