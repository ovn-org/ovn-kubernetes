...
new routing table specifically created to route the traffic out the correct interface. IPTables are also altered and an entry
...
Routes within the main table which egress the interface picked to host the Egress IP are also added to this routing table.
...
to the main routing table at a priority higher that the EgressIP IP rules, which are set to priority `6000`:
```shell
5999:	from all fwmark 0x3f0 lookup main
```
Note: `0x3f0` is `1008` in hexadecimal.
