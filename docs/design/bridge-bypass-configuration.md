...
- Specify `NETDEV` as gateway interface explicitly via `OVN_GATEWAY_OPTS` environment variable for
...
must be configured to create ovnkube dedicated resource pool. Name of the resource passed via
`OVNKUBE_NODE_BYPASS_PORT_DP_RESOURCE_NAME` environment variable to ovnkube-node container and
requested using `resources` field. Example:
...
Ovnkube can transit from regular bridge configuration to bypass bridge configuration, but opposite
is not possible due to the design, though this can change in the future.
