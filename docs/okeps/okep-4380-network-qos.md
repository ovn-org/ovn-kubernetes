...
- Provide a mechanism for users to set DSCP marking on east west and north south traffic.
- Provide a mechanism for users to set Metering on east west and north south traffic.
- Make NetworkQoS CRD fields to be more extensible.
...
As a user of OVN-Kubernetes, I should be able to mark egress traffic coming from a specific namespace with a valid DSCP value.

#### Story 2

As a user of OVN-Kubernetes, I should be able to apply bandwidth limit (rate and burst) for a pod traffic originating from a specific namespace.

#### Story 3

As a user of OVN-Kubernetes, I should be able to create multiple NetworkQoS objects per namesapce containing rules with different priorities.

#### Story 4

The NetworkQoS Status field must reflect observed state of the object covering entire Kubernetes cluster.

#### Story 5

As a user of OVN-Kubernetes, I should be able to apply DSCP and/or bandwidth limit on Secondary Networks of a Multi-homed Pod.
...
the equivalent of:
```bash
ovn-nbctl qos-add node1 to-lport 1000 "ip4.src == <default_ns-qos-external-foo address set> && ip4.dst == 1.2.3.4/32 && tcp && tcp.dst == 22" dscp=59
ovn-nbctl qos-add node1 to-lport 999 "ip4.src == <default_ns-qos-external-foo address set> && ip4.dst == 1.2.3.4/32" dscp=46
      ...
      - podSelector:
        ...
        namespaceSelector:
          matchLabels:
            name: stream-target
```
In the above example, The tcp traffic with dst port 471 from pods with label `app=storage-client` in `games` namespace will have dscp set to 59 and bandwidth limited to specified burst/rate towards pods having label `app=storage-target` residing on the namespace with label `name=storage-target`.
The udp traffic with dst port 399 from pods with label `app=stream-client` in `games` namespace will have dscp set to 33 towards pods with label `app=stream-client` residing on the namespace with label `name=stream-target`.
